...
/*/
//--------------------------------------------------------------------
User Function UP48035( cEmpAmb, cFilAmb )
Local   aSay      := {}
Local   aButton   := {}
Local   aMarcadas := {}
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgInfo( "Atualização realizada.", "UP48035" )
				Else
					MsgStop( "Atualização não realizada.", "UP48035" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'ZF5'																	, .T. }, ; //X3_ARQUIVO
	{ '9C'																	, .T. }, ; //X3_ORDEM
	{ 'ZF5_CHVNDT'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 14																	, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Chave NDT'															, .T. }, ; //X3_TITULO
	{ 'Chave NDT'															, .T. }, ; //X3_TITSPA
	{ 'Chave NDT'															, .T. }, ; //X3_TITENG
	{ 'Chave NDT'															, .T. }, ; //X3_DESCRIC
	{ 'Chave NDT'															, .T. }, ; //X3_DESCSPA
	{ 'Chave NDT'															, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP48035" ) ) ) ;
