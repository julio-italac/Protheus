...
/*/
//--------------------------------------------------------------------
User Function UP46480A( cEmpAmb, cFilAmb )
Local   aSay      := {}
Local   aButton   := {}
Local   aMarcadas := {}
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgInfo( "Atualização realizada.", "UP46480A" )
				Else
					MsgStop( "Atualização não realizada.", "UP46480A" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	...
	{ 'Vlr Pedagio'															, .T. }, ; //X3_TITSPA
	{ 'Vlr Pedagio'															, .T. }, ; //X3_TITENG
...
aAdd( aSX3, { ;
	...
	{ 'Vlr Chep'															, .T. }, ; //X3_TITSPA
	{ 'Vlr Chep'															, .T. }, ; //X3_TITENG
	{ 'Valor do CHEP'														, .T. }, ; //X3_DESCRIC
	{ 'Valor do CHEP'														, .T. }, ; //X3_DESCSPA
	{ 'Valor do CHEP'														, .T. }, ; //X3_DESCENG
	{ '@E 999,999,999.99999'												, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	{ 'x       x       x       x       x       x       x       x       x       x       x       x       x       x       x', .T. }, ; //X3_USADO
	{ ''																	, .T. }, ; //X3_RELACAO
...
aAdd( aSX3, { ;
	...
	{ 'Vlr Frete OL'														, .T. }, ; //X3_TITSPA
	{ 'Vlr Frete OL'														, .T. }, ; //X3_TITENG
	{ 'Valor do Frete OL'													, .T. }, ; //X3_DESCRIC
	{ 'Valor do Frete OL'													, .T. }, ; //X3_DESCSPA
	{ 'Valor do Frete OL'													, .T. }, ; //X3_DESCENG
	{ '@E 999,999,999.99999'												, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	{ 'x       x       x       x       x       x       x       x       x       x       x       x       x       x       x', .T. }, ; //X3_USADO
	{ ''																	, .T. }, ; //X3_RELACAO
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP46480A" ) ) ) ;
