...
/*/
//--------------------------------------------------------------------
User Function UP25693( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UP25693" )
				Else
					MsgStop( "Atualização não Realizada.", "UP25693" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'ZFQ'																	, .T. }, ; //X3_ARQUIVO
	{ '71'																	, .T. }, ; //X3_ORDEM
	{ 'ZFQ_FILRDC'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 4																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Cod.Fil.RDC'															, .T. }, ; //X3_TITULO
	{ 'Cod.Fil.RDC'															, .T. }, ; //X3_TITSPA
	{ 'Cod.Fil.RDC'															, .T. }, ; //X3_TITENG
	{ 'Codigo da Filial RDC'												, .T. }, ; //X3_DESCRIC
	{ 'Codigo da Filial RDC'												, .T. }, ; //X3_DESCSPA
	{ 'Codigo da Filial RDC'												, .T. }, ; //X3_DESCENG
	{ '9999'																, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
//
aHlpPor := {}
aAdd( aHlpPor, 'Código da filial de envio a sistema RDC.' )

PutHelp( "PZFQ_FILRDC", aHlpPor, {}, {}, .T. )
AutoGrLog( "Atualizado o Help do campo " + "ZFQ_FILRDC" )

//
// Helps Tabela ZG9
//
aHlpPor := {}
aAdd( aHlpPor, 'Código da filial de envio ao sistema' )
aAdd( aHlpPor, 'RDC.' )

PutHelp( "PZG9_FILRDC", aHlpPor, {}, {}, .T. )
AutoGrLog( "Atualizado o Help do campo " + "ZG9_FILRDC" )
...
PutHelp( "PZZM_FILRDC", aHlpPor, {}, {}, .T. )
AutoGrLog( "Atualizado o Help do campo " + "ZZM_FILRDC" )
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP25693" ) ) ) ;
