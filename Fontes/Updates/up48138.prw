...
/*/
//--------------------------------------------------------------------
User Function UP48138( cEmpAmb, cFilAmb )
Local   aSay      := {}
Local   aButton   := {}
Local   aMarcadas := {}
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgInfo( "Atualização realizada.", "UP48138" )
				Else
					MsgStop( "Atualização não realizada.", "UP48138" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	...
	{ 'IF(INCLUI,"",POSICIONE("ZEL",1,xFilial("ZEL")+ZG5->ZG5_LOCEMB,"ZEL_DESCRI"))', .T. }, ; //X3_RELACAO
	{ 'F3ITLC'																, .T. }, ; //X3_F3
	{ 0																		, .T. }, ; //X3_NIVEL
	...
	{ ''																	, .T. }, ; //X3_PICTVAR
	{ ''																	, .T. }, ; //X3_WHEN
	{ 'POSICIONE("ZEL",1,xFilial("ZEL")+ZG5->ZG5_LOCEMB,"ZEL_DESCRI")'		, .T. }, ; //X3_INIBRW
	{ ''																	, .T. }, ; //X3_GRPSXG
	{ '1'																	, .T. }, ; //X3_FOLDER
	{ ''																	, .T. }, ; //X3_CONDSQL
	{ ''																	, .T. }, ; //X3_CHKSQL
...
aAdd( aSIX, { ;
	'ZG5'																	, ; //INDICE
	'5'																		, ; //ORDEM
	'ZG5_FILIAL+ZG5_LOCEMB+ZG5_UF+ZG5_CODMUN+ZG5_MESO+ZG5_MICRO'			, ; //CHAVE
	'Loc.Embarque+UF+Cod.Municipio+Meso+Micro'								, ; //DESCRICAO
	''																		, ; //DESCSPA
	''																		, ; //DESCENG
	'U'																		, ; //PROPRI
	''																		, ; //F3
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP48138" ) ) ) ;
