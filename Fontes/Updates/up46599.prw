...
/*/
//--------------------------------------------------------------------
User Function UP46599( cEmpAmb, cFilAmb )
Local   aSay      := {}
Local   aButton   := {}
Local   aMarcadas := {}
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgInfo( "Atualização realizada.", "UP46599" )
				Else
					MsgStop( "Atualização não realizada.", "UP46599" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'CC2'																	, .T. }, ; //X3_ARQUIVO
	{ '24'																	, .T. }, ; //X3_ORDEM
	{ 'CC2_I_MELO'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 6																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Meso Logisti'														, .T. }, ; //X3_TITULO
	{ 'Meso Logisti'														, .T. }, ; //X3_TITSPA
	{ 'Meso Logisti'														, .T. }, ; //X3_TITENG
	{ 'Grupo da Meso Logistica'												, .T. }, ; //X3_DESCRIC
	{ 'Grupo da Meso Logistica'												, .T. }, ; //X3_DESCSPA
	{ 'Grupo da Meso Logistica'												, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ 'Vazio() .Or. ExistCpo("Z21",M->CC2_I_MELO,1)'						, .T. }, ; //X3_VALID
...
aAdd( aSX3, { ;
	{ 'ZEL'																	, .T. }, ; //X3_ARQUIVO
	{ '14'																	, .T. }, ; //X3_ORDEM
	{ 'ZEL_PMAXOL'															, .T. }, ; //X3_CAMPO
	{ 'N'																	, .T. }, ; //X3_TIPO
	{ 5																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'PesoMax.OpLo'														, .T. }, ; //X3_TITULO
	{ 'PesoMax.OpLo'														, .T. }, ; //X3_TITSPA
	{ 'PesoMax.OpLo'														, .T. }, ; //X3_TITENG
	{ 'Peso Máximo Operador Logí'											, .T. }, ; //X3_DESCRIC
	{ 'Peso Máximo Operador Logí'											, .T. }, ; //X3_DESCSPA
	{ 'Peso Máximo Operador Logí'											, .T. }, ; //X3_DESCENG
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP46599" ) ) ) ;
