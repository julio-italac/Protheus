...
/*/
//--------------------------------------------------------------------
User Function UP46064( cEmpAmb, cFilAmb )
Local   aSay      := {}
Local   aButton   := {}
Local   aMarcadas := {}
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização realizada.", "UP46064" )
				Else
					MsgStop( "Atualização não realizada.", "UP46064" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	...
	{ 'Protocolo TMS'														, .F. }, ; //X3_DESCRIC
	{ 'Protocolo TMS'														, .F. }, ; //X3_DESCSPA
	{ 'Protocolo TMS'														, .F. }, ; //X3_DESCENG
	{ ''																	, .F. }, ; //X3_PICTURE
	{ ''																	, .F. }, ; //X3_VALID
...
aAdd( aSX3, { ;
	{ 'ZFJ'																	, .F. }, ; //X3_ARQUIVO
	{ '29'																	, .F. }, ; //X3_ORDEM
	{ 'ZFJ_FILTMS'															, .F. }, ; //X3_CAMPO
	{ 'C'																	, .F. }, ; //X3_TIPO
	{ 4																		, .F. }, ; //X3_TAMANHO
	{ 0																		, .F. }, ; //X3_DECIMAL
	{ 'Filial TMS'															, .F. }, ; //X3_TITULO
	{ 'Filial TMS'															, .F. }, ; //X3_TITSPA
	{ 'Filial TMS'															, .F. }, ; //X3_TITENG
	{ 'Filial TMS'															, .F. }, ; //X3_DESCRIC
	{ 'Filial TMS'															, .F. }, ; //X3_DESCSPA
	{ 'Filial TMS'															, .F. }, ; //X3_DESCENG
	{ ''																	, .F. }, ; //X3_PICTURE
	{ ''																	, .F. }, ; //X3_VALID
	{ 'x       x       x       x       x       x       x       x       x       x       x       x       x       x       x x', .F. }, ; //X3_USADO
	{ ''																	, .F. }, ; //X3_RELACAO
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP46064" ) ) ) ;
