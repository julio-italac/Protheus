...
/*/
//--------------------------------------------------------------------
User Function UP50197( cEmpAmb, cFilAmb )
Local   aSay      := {}
Local   aButton   := {}
Local   aMarcadas := {}
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgInfo( "Atualização realizada.", "UP50197" )
				Else
					MsgStop( "Atualização não realizada.", "UP50197" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'SC5'																	, .T. }, ; //X3_ARQUIVO
	{ 'C6'																	, .T. }, ; //X3_ORDEM
	{ 'C5_I_BLSLD'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Bloq.p/Saldo'														, .T. }, ; //X3_TITULO
	{ 'Bloq.p/Saldo'														, .T. }, ; //X3_TITSPA
	{ 'Bloq.p/Saldo'														, .T. }, ; //X3_TITENG
	{ 'Bloq. de carregamentro p/'											, .T. }, ; //X3_DESCRIC
	{ 'Bloq. de carregamentro p/'											, .T. }, ; //X3_DESCSPA
	{ 'Bloq. de carregamentro p/'											, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP50197" ) ) ) ;
