...
/*/
//--------------------------------------------------------------------
User Function UP37679( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UP37679" )
				Else
					MsgStop( "Atualização não Realizada.", "UP37679" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'ZF5'																	, .T. }, ; //X3_ARQUIVO
	{ '95'																	, .T. }, ; //X3_ORDEM
	{ 'ZF5_MERENT'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Merc.Entreg'															, .T. }, ; //X3_TITULO
	{ 'Merc.Entreg'															, .T. }, ; //X3_TITSPA
	{ 'Merc.Entreg'															, .T. }, ; //X3_TITENG
	{ 'Mercadoria Entregue?'												, .T. }, ; //X3_DESCRIC
	{ 'Mercadoria Entregue?'												, .T. }, ; //X3_DESCSPA
	{ 'Mercadoria Entregue?'												, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
aAdd( aSX3, { ;
	{ 'ZF5'																	, .T. }, ; //X3_ARQUIVO
	{ '96'																	, .T. }, ; //X3_ORDEM
	{ 'ZF5_SITENT'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Sit.Entrega'															, .T. }, ; //X3_TITULO
	{ 'Sit.Entrega'															, .T. }, ; //X3_TITSPA
	{ 'Sit.Entrega'															, .T. }, ; //X3_TITENG
	{ 'Situacao da Entrega'													, .T. }, ; //X3_DESCRIC
	{ 'Situacao da Entrega'													, .T. }, ; //X3_DESCSPA
	{ 'Situacao da Entrega'													, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	{ 'x       x       x       x       x       x       x       x       x       x       x       x       x       x       x x', .T. }, ; //X3_USADO
	{ ''																	, .T. }, ; //X3_RELACAO
	...
	{ 'R'																	, .T. }, ; //X3_CONTEXT
	{ ''																	, .T. }, ; //X3_OBRIGAT
	{ 'Pertence("IP ")'														, .T. }, ; //X3_VLDUSER
	{ 'I=Integral;P=Parcial'												, .T. }, ; //X3_CBOX
	{ ''																	, .T. }, ; //X3_CBOXSPA
	{ ''																	, .T. }, ; //X3_CBOXENG
	{ ''																	, .T. }, ; //X3_PICTVAR
	{ 'M->ZF5_MERENT=="S"'													, .T. }, ; //X3_WHEN
	{ ''																	, .T. }, ; //X3_INIBRW
	{ ''																	, .T. }, ; //X3_GRPSXG
	{ '1'																	, .T. }, ; //X3_FOLDER
	{ ''																	, .T. }, ; //X3_CONDSQL
...
//
aHlpPor := {}
aAdd( aHlpPor, 'Mercadoria Entregue?' )

PutSX1Help( "PZF5_MERENT", aHlpPor, {}, {}, .T.,,.T. )
AutoGrLog( "Atualizado o Help do campo " + "ZF5_MERENT" )

aHlpPor := {}
aAdd( aHlpPor, 'Situação da Entrega.' )

PutSX1Help( "PZF5_SITENT", aHlpPor, {}, {}, .T.,,.T. )
AutoGrLog( "Atualizado o Help do campo " + "ZF5_SITENT" )
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP37679" ) ) ) ;
