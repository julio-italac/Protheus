...
/*/
//--------------------------------------------------------------------
User Function UP51635( cEmpAmb, cFilAmb )
Local   aSay      := {}
Local   aButton   := {}
Local   aMarcadas := {}
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgInfo( "Atualização realizada.", "UP51635" )
				Else
					MsgStop( "Atualização não realizada.", "UP51635" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	...
	{ 'R'																	, .T. }, ; //X3_CONTEXT
	{ 'x'																	, .T. }, ; //X3_OBRIGAT
	{ 'UsrExist(M->ZY0_CODUSR) .AND. U_AOMS071VC()'							, .T. }, ; //X3_VLDUSER
	{ ''																	, .T. }, ; //X3_CBOX
	{ ''																	, .T. }, ; //X3_CBOXSPA
	{ ''																	, .T. }, ; //X3_CBOXENG
	{ ''																	, .T. }, ; //X3_PICTVAR
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP51635" ) ) ) ;
