...
/*/
//--------------------------------------------------------------------
User Function UP29840( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UP29840" )
				Else
					MsgStop( "Atualização não Realizada.", "UP29840" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'SZW'																	, .T. }, ; //X3_ARQUIVO
	{ 'C1'																	, .T. }, ; //X3_ORDEM
	{ 'ZW_ENVMAIL'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Enviado Mail'														, .T. }, ; //X3_TITULO
	{ 'Enviado Mail'														, .T. }, ; //X3_TITSPA
	{ 'Enviado Mail'														, .T. }, ; //X3_TITENG
	{ 'Enviado E-mail'														, .T. }, ; //X3_DESCRIC
	{ 'Enviado E-mail'														, .T. }, ; //X3_DESCSPA
	{ 'Enviado E-mail'														, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP29840" ) ) ) ;
