...
/*/
//--------------------------------------------------------------------
User Function UPD44881( cEmpAmb, cFilAmb )
Local   aSay      := {}
Local   aButton   := {}
Local   aMarcadas := {}
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização realizada.", "UPD44881" )
				Else
					MsgStop( "Atualização não realizada.", "UPD44881" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'ZY5'																	, .T. }, ; //X3_ARQUIVO
	{ '06'																	, .T. }, ; //X3_ORDEM
	{ 'ZY5_CAMPOS'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 50																	, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Filtro Campo'														, .T. }, ; //X3_TITULO
	{ 'Filtro Campo'														, .T. }, ; //X3_TITSPA
	{ 'Filtro Campo'														, .T. }, ; //X3_TITENG
	{ 'Filtro por Campo'													, .T. }, ; //X3_DESCRIC
	{ 'Filtro por Campo'													, .T. }, ; //X3_DESCSPA
	{ 'Filtro por Campo'													, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPD44881" ) ) ) ;
