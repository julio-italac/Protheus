...
/*/
//--------------------------------------------------------------------
User Function UP47390( cEmpAmb, cFilAmb )
Local   aSay      := {}
Local   aButton   := {}
Local   aMarcadas := {}
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização realizada.", "UP47390" )
				Else
					MsgStop( "Atualização não realizada.", "UP47390" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'SC5'																	, .T. }, ; //X3_ARQUIVO
	{ 'ET'																	, .T. }, ; //X3_ORDEM
	{ 'C5_I_DIASV'															, .T. }, ; //X3_CAMPO
	{ 'N'																	, .T. }, ; //X3_TIPO
	{ 5																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Dias Viagem'															, .T. }, ; //X3_TITULO
	{ 'Dias Viagem'															, .T. }, ; //X3_TITSPA
	{ 'Dias Viagem'															, .T. }, ; //X3_TITENG
	{ 'Dias Viagem'															, .T. }, ; //X3_DESCRIC
	{ 'Dias Viagem'															, .T. }, ; //X3_DESCSPA
	{ 'Dias Viagem'															, .T. }, ; //X3_DESCENG
...
aAdd( aSX3, { ;
	{ 'SC5'																	, .T. }, ; //X3_ARQUIVO
	{ 'EU'																	, .T. }, ; //X3_ORDEM
	{ 'C5_I_DIASO'															, .T. }, ; //X3_CAMPO
	{ 'N'																	, .T. }, ; //X3_TIPO
	{ 5																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Dias Oper.'															, .T. }, ; //X3_TITULO
	{ 'Dias Oper.'															, .T. }, ; //X3_TITSPA
	{ 'Dias Oper.'															, .T. }, ; //X3_TITENG
	{ 'Dias Operacional'													, .T. }, ; //X3_DESCRIC
	{ 'Dias Operacional'													, .T. }, ; //X3_DESCSPA
	{ 'Dias Operacional'													, .T. }, ; //X3_DESCENG
	{ '@E 99,999'															, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	{ 'x       x       x       x       x       x       x       x       x       x       x       x       x       x       x x', .T. }, ; //X3_USADO
	{ ''																	, .T. }, ; //X3_RELACAO
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP47390" ) ) ) ;
