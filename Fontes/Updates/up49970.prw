...
/*/
//--------------------------------------------------------------------
User Function UP49970( cEmpAmb, cFilAmb )
Local   aSay      := {}
Local   aButton   := {}
Local   aMarcadas := {}
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgInfo( "Atualização realizada.", "UP49970" )
				Else
					MsgStop( "Atualização não realizada.", "UP49970" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	...
	{ ''																	, .T. }, ; //X3_TITSPA
	{ ''																	, .T. }, ; //X3_TITENG
	{ 'Aliquota do ICMS Estadual'											, .T. }, ; //X3_DESCRIC
	{ ''																	, .T. }, ; //X3_DESCSPA
	{ ''																	, .T. }, ; //X3_DESCENG
...
aAdd( aSX3, { ;
	...
	{ ''																	, .T. }, ; //X3_TITSPA
	{ ''																	, .T. }, ; //X3_TITENG
	{ 'Valor do ICMS Estadual'												, .T. }, ; //X3_DESCRIC
	{ ''																	, .T. }, ; //X3_DESCSPA
	{ ''																	, .T. }, ; //X3_DESCENG
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP49970" ) ) ) ;
