...
/*/
//--------------------------------------------------------------------
User Function UPD30816( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UPD30816" )
				Else
					MsgStop( "Atualização não Realizada.", "UPD30816" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'SC1'																	, .T. }, ; //X3_ARQUIVO
	{ '09'																	, .T. }, ; //X3_ORDEM
	{ 'C1_I_ORPRU'															, .T. }, ; //X3_CAMPO
	{ 'N'																	, .T. }, ; //X3_TIPO
	{ 15																	, .T. }, ; //X3_TAMANHO
	{ 2																		, .T. }, ; //X3_DECIMAL
	{ 'Prc.Unit.Orc'														, .T. }, ; //X3_TITULO
	{ 'Prc.Unit.Orc'														, .T. }, ; //X3_TITSPA
	{ 'Prc.Unit.Orc'														, .T. }, ; //X3_TITENG
	{ 'Preco Initario Orcado'												, .T. }, ; //X3_DESCRIC
	{ 'Preco Initario Orcado'												, .T. }, ; //X3_DESCSPA
	{ 'Preco Initario Orcado'												, .T. }, ; //X3_DESCENG
...
aAdd( aSX3, { ;
	{ 'SC1'																	, .T. }, ; //X3_ARQUIVO
	{ '09'																	, .T. }, ; //X3_ORDEM
	{ 'C1_I_ORTOT'															, .T. }, ; //X3_CAMPO
	{ 'N'																	, .T. }, ; //X3_TIPO
	{ 15																	, .T. }, ; //X3_TAMANHO
	{ 2																		, .T. }, ; //X3_DECIMAL
	{ 'Prc.Tot.Orca'														, .T. }, ; //X3_TITULO
	{ 'Prc.Tot.Orca'														, .T. }, ; //X3_TITSPA
	{ 'Prc.Tot.Orca'														, .T. }, ; //X3_TITENG
	{ 'Preco Total Orcado'													, .T. }, ; //X3_DESCRIC
	{ 'Preco Total Orcado'													, .T. }, ; //X3_DESCSPA
	{ 'Preco Total Orcado'													, .T. }, ; //X3_DESCENG
	{ '@E 999,999,999,999.99'												, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	{ Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) + ;
	Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) + ;
...
aAdd( aSX7, { ;
	'C1_I_ORPRU'															, ; //X7_CAMPO
	'001'																	, ; //X7_SEQUENC
	'M->C1_I_ORTOT := M->C1_I_ORPRU*M->C1_QUANT'							, ; //X7_REGRA
	'C1_I_ORTOT'															, ; //X7_CDOMIN
	'P'																		, ; //X7_TIPO
	'N'																		, ; //X7_SEEK
	''																		, ; //X7_ALIAS
	0																		, ; //X7_ORDEM
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPD30816" ) ) ) ;
