...
/*/
//--------------------------------------------------------------------
User Function UPD34937( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UPD34937" )
				Else
					MsgStop( "Atualização não Realizada.", "UPD34937" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'ZZL'																	, .T. }, ; //X3_ARQUIVO
	{ '99'																	, .T. }, ; //X3_ORDEM
	{ 'ZZL_TABPRD'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 250																	, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Tab Preco DW'														, .T. }, ; //X3_TITULO
	{ 'Tab Preco DW'														, .T. }, ; //X3_TITSPA
	{ 'Tab Preco DW'														, .T. }, ; //X3_TITENG
	{ 'Tabelas de preco portal d'											, .T. }, ; //X3_DESCRIC
	{ 'Tabelas de preco portal d'											, .T. }, ; //X3_DESCSPA
	{ 'Tabelas de preco portal d'											, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPD34937" ) ) ) ;
