...
/*/
//--------------------------------------------------------------------
User Function UP40074( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UP40074" )
				Else
					MsgStop( "Atualização não Realizada.", "UP40074" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'ZF5'																	, .T. }, ; //X3_ARQUIVO
	{ '03'																	, .T. }, ; //X3_ORDEM
	{ 'ZF5_DTOCOR'															, .T. }, ; //X3_CAMPO
	{ 'D'																	, .T. }, ; //X3_TIPO
	{ 8																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Data Ocorren'														, .T. }, ; //X3_TITULO
	{ 'Data Ocorren'														, .T. }, ; //X3_TITSPA
	{ 'Data Ocorren'														, .T. }, ; //X3_TITENG
	{ 'Data da Ocorrencia Frete'											, .T. }, ; //X3_DESCRIC
	{ 'Data da Ocorrencia Frete'											, .T. }, ; //X3_DESCSPA
	{ 'Data da Ocorrencia Frete'											, .T. }, ; //X3_DESCENG
	{ ''																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
For nI := 1 To Len( aSX3 )
	...
	If !SX3->( dbSeek( PadR( aSX3[nI][nPosCpo][1], nTamSeek ) ) )
		...
		For nJ := 1 To Len( aSX3[nI] )
			If     nJ == nPosOrd  // Ordem
				SX3->( FieldPut( FieldPos( aEstrut[nJ][1] ), aSX3[nI][nJ][1] ) ) // cSeqAtu
...
//
aHlpPor := {}
aAdd( aHlpPor, 'Data da Ocorrência de Frete.' )

PutSX1Help( "PZF5_DTOCOR", aHlpPor, {}, {}, .T.,,.T. )
AutoGrLog( "Atualizado o Help do campo " + "ZF5_DTOCOR" )
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP40074" ) ) ) ;
