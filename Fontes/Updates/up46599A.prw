...
/*/
//--------------------------------------------------------------------
User Function UP46599A( cEmpAmb, cFilAmb )
Local   aSay      := {}
Local   aButton   := {}
Local   aMarcadas := {}
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgInfo( "Atualização realizada.", "UPD46599" )
				Else
					MsgStop( "Atualização não realizada.", "UPD46599" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'ZEL'																	, .T. }, ; //X3_ARQUIVO
	{ '12'																	, .T. }, ; //X3_ORDEM
	{ 'ZEL_MULPAL'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 50																	, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Mult.Pallets'														, .T. }, ; //X3_TITULO
	{ 'Mult.Pallets'														, .T. }, ; //X3_TITSPA
	{ 'Mult.Pallets'														, .T. }, ; //X3_TITENG
	{ 'Multiplo de Pallets'													, .T. }, ; //X3_DESCRIC
	{ 'Multiplo de Pallets'													, .T. }, ; //X3_DESCSPA
	{ 'Multiplo de Pallets'													, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ '          '															, .T. }, ; //X3_VALID
...
aAdd( aSX3, { ;
	{ 'ZEL'																	, .T. }, ; //X3_ARQUIVO
	{ '13'																	, .T. }, ; //X3_ORDEM
	{ 'ZEL_MULPES'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 50																	, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Mult.Pesos'															, .T. }, ; //X3_TITULO
	{ 'Mult.Pesos'															, .T. }, ; //X3_TITSPA
	{ 'Mult.Pesos'															, .T. }, ; //X3_TITENG
	{ 'Multiplo de Pesos'													, .T. }, ; //X3_DESCRIC
	{ 'Multiplo de Pesos'													, .T. }, ; //X3_DESCSPA
	{ 'Multiplo de Pesos'													, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ '          '															, .T. }, ; //X3_VALID
	{ 'x       x       x       x       x       x       x       x       x       x       x       x       x       x       x x', .T. }, ; //X3_USADO
	{ ''																	, .T. }, ; //X3_RELACAO
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPD46599" ) ) ) ;
