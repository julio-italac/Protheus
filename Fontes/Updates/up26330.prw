...
/*/
//--------------------------------------------------------------------
User Function UP26330( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UP26330" )
				Else
					MsgStop( "Atualização não Realizada.", "UP26330" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	...
	{ 'Data de Envio RDC'													, .T. }, ; //X3_DESCRIC
	{ 'Data de Envio RDC'													, .T. }, ; //X3_DESCSPA
	{ 'Data de Envio RDC'													, .T. }, ; //X3_DESCENG
	{ ''																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
aAdd( aSX3, { ;
	...
	{ 'Hora de Envio RDC'													, .T. }, ; //X3_DESCRIC
	{ 'Hora de Envio RDC'													, .T. }, ; //X3_DESCSPA
	{ 'Hora de Envio RDC'													, .T. }, ; //X3_DESCENG
	{ ''																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	{ Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) + ;
	Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) + ;
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP26330" ) ) ) ;
