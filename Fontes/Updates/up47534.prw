...
/*/
//--------------------------------------------------------------------
User Function UP47534( cEmpAmb, cFilAmb )
Local   aSay      := {}
Local   aButton   := {}
Local   aMarcadas := {}
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização realizada.", "UP47534" )
				Else
					MsgStop( "Atualização não realizada.", "UP47534" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	...
	{ 'IIF(INCLUI,' + DUPLAS  + '' + DUPLAS  + ',POSICIONE(' + DUPLAS  + 'CC3' + DUPLAS  + ',1,XFILIAL(' + SIMPLES + 'CC3' + SIMPLES + ')+M->A1_CNAE,' + DUPLAS  + 'CC3_DESC' + DUPLAS  + '))', .T. }, ; //X3_RELACAO
	{ ''																	, .T. }, ; //X3_F3
	{ 0																		, .T. }, ; //X3_NIVEL
	...
	{ ''																	, .T. }, ; //X3_PICTVAR
	{ ''																	, .T. }, ; //X3_WHEN
	{ 'POSICIONE(' + DUPLAS  + 'CC3' + DUPLAS  + ',1,XFILIAL(' + SIMPLES + 'CC3' + SIMPLES + ')+SA1->A1_CNAE,' + DUPLAS  + 'CC3_DESC' + DUPLAS  + ')', .T. }, ; //X3_INIBRW
	{ ''																	, .T. }, ; //X3_GRPSXG
	{ '1'																	, .T. }, ; //X3_FOLDER
	{ ''																	, .T. }, ; //X3_CONDSQL
	{ ''																	, .T. }, ; //X3_CHKSQL
...
aAdd( aSX3, { ;
	...
	{ ''																	, .T. }, ; //X3_VALID
	{ 'x       x       x       x       x       x       x       x       x       x       x       x       x       x       x x', .T. }, ; //X3_USADO
	{ 'IIF(INCLUI,' + DUPLAS  + '' + DUPLAS  + ',POSICIONE(' + DUPLAS  + 'CC3' + DUPLAS  + ',1,XFILIAL(' + SIMPLES + 'CC3' + SIMPLES + ')+M->ZX_I_END,' + DUPLAS  + 'CC3_DESC' + DUPLAS  + '))', .T. }, ; //X3_RELACAO
	{ ''																	, .T. }, ; //X3_F3
	{ 0																		, .T. }, ; //X3_NIVEL
	{ 'xxxxxx x'															, .T. }, ; //X3_RESERV
	{ ''																	, .T. }, ; //X3_CHECK
	...
	{ ''																	, .T. }, ; //X3_PICTVAR
	{ ''																	, .T. }, ; //X3_WHEN
	{ 'POSICIONE(' + DUPLAS  + 'CC3' + DUPLAS  + ',1,XFILIAL(' + SIMPLES + 'CC3' + SIMPLES + ')+SZX->ZX_I_END,' + DUPLAS  + 'CC3_DESC' + DUPLAS  + ')', .T. }, ; //X3_INIBRW
	{ ''																	, .T. }, ; //X3_GRPSXG
	{ ''																	, .T. }, ; //X3_FOLDER
	{ ''																	, .T. }, ; //X3_CONDSQL
	{ ''																	, .T. }, ; //X3_CHKSQL
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP47534" ) ) ) ;
