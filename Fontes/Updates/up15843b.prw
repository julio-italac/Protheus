...
/*/
//--------------------------------------------------------------------
User Function UP15843B( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UP15843B" )
				Else
					MsgStop( "Atualização não Realizada.", "UP15843B" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'Z18'																	, .T. }, ; //X3_ARQUIVO
	{ '20'																	, .T. }, ; //X3_ORDEM
	{ 'Z18_DSCALT'															, .T. }, ; //X3_CAMPO
	{ 'M'																	, .T. }, ; //X3_TIPO
	{ 10																	, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Desc.Alterac'														, .T. }, ; //X3_TITULO
	{ 'Desc.Alterac'														, .T. }, ; //X3_TITSPA
	{ 'Desc.Alterac'														, .T. }, ; //X3_TITENG
	{ 'Descricao das Alteracoes'											, .T. }, ; //X3_DESCRIC
	{ 'Descricao das Alteracoes'											, .T. }, ; //X3_DESCSPA
	{ 'Descricao das Alteracoes'											, .T. }, ; //X3_DESCENG
	{ ''																	, .T. }, ; //X3_PICTURE
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP15843B" ) ) ) ;
