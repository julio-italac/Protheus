...
/*/
//--------------------------------------------------------------------
User Function UP51757( cEmpAmb, cFilAmb )
Local   aSay      := {}
Local   aButton   := {}
Local   aMarcadas := {}
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgInfo( "Atualização realizada.", "UP51757" )
				Else
					MsgStop( "Atualização não realizada.", "UP51757" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	...
	1																		, ; //X3_TAMANHO
	0																		, ; //X3_DECIMAL
	'Portal Clien'															, ; //X3_TITULO
	'Portal Clien'															, ; //X3_TITSPA
	'Portal Clien'															, ; //X3_TITENG
	'Portal do Cliente?'													, ; //X3_DESCRIC
	'Portal do Cliente?'													, ; //X3_DESCSPA
	'Portal do Cliente?'													, ; //X3_DESCENG
	'@!'																	, ; //X3_PICTURE
	''																		, ; //X3_VALID
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP51757" ) ) ) ;
