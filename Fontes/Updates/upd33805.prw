...
/*/
//--------------------------------------------------------------------
User Function UPD33805( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UPD33805" )
				Else
					MsgStop( "Atualização não Realizada.", "UPD33805" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSIX, { ;
	'ZAG'																	, ; //INDICE
	'1'																		, ; //ORDEM
	'ZAG_FILIAL+ZAG_DATA+ZAG_CLIENT+ZAG_LOJA'								, ; //CHAVE
	'Data+Cliente+Loja'														, ; //DESCRICAO
	''																		, ; //DESCSPA
	''																		, ; //DESCENG
	'U'																		, ; //PROPRI
	''																		, ; //F3
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPD33805" ) ) ) ;
