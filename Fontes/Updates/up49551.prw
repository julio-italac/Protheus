...
/*/
//--------------------------------------------------------------------
User Function UP49551( cEmpAmb, cFilAmb )
Local   aSay      := {}
Local   aButton   := {}
Local   aMarcadas := {}
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgInfo( "Atualização realizada.", "UP49551" )
				Else
					MsgStop( "Atualização não realizada.", "UP49551" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'DAK'																	, .T. }, ; //X3_ARQUIVO
	{ '9F'																	, .T. }, ; //X3_ORDEM
	{ 'DAK_I_TMS'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'TMS orig car'														, .T. }, ; //X3_TITULO
	{ 'TMS orig car'														, .T. }, ; //X3_TITSPA
	{ 'TMS orig car'														, .T. }, ; //X3_TITENG
	{ 'TMS origem da carga'													, .T. }, ; //X3_DESCRIC
	{ 'TMS origem da carga'													, .T. }, ; //X3_DESCSPA
	{ 'TMS origem da carga'													, .T. }, ; //X3_DESCENG
	{ ''																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	...
	{ ''																	, .T. }, ; //X3_OBRIGAT
	{ ''																	, .T. }, ; //X3_VLDUSER
	{ 'M=Multiembarcador;R=RDC'												, .T. }, ; //X3_CBOX
	{ 'M=Multiembarcador;R=RDC'												, .T. }, ; //X3_CBOXSPA
	{ 'M=Multiembarcador;R=RDC'												, .T. }, ; //X3_CBOXENG
	{ ''																	, .T. }, ; //X3_PICTVAR
	{ ''																	, .T. }, ; //X3_WHEN
	{ ''																	, .T. }, ; //X3_INIBRW
	{ ''																	, .T. }, ; //X3_GRPSXG
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP49551" ) ) ) ;
