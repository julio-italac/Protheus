...
/*/
//--------------------------------------------------------------------
User Function UP47782( cEmpAmb, cFilAmb )
Local   aSay      := {}
Local   aButton   := {}
Local   aMarcadas := {}
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UP47782" )
				Else
					MsgStop( "Atualização não Realizada.", "UP47782" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'SA2'																	, .T. }, ; //X3_ARQUIVO
	{ 'FV'																	, .T. }, ; //X3_ORDEM
	{ 'A2_I_TPAVE'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Tipo Averb C'														, .T. }, ; //X3_TITULO
	{ 'Tipo Averb C'														, .T. }, ; //X3_TITSPA
	{ 'Tipo Averb C'														, .T. }, ; //X3_TITENG
	{ 'Tipo Averbacao de Carga'												, .T. }, ; //X3_DESCRIC
	{ 'Tipo Averbacao de Carga'												, .T. }, ; //X3_DESCSPA
	{ 'Tipo Averbacao de Carga'												, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	...
	{ 'R'																	, .T. }, ; //X3_CONTEXT
	{ ''																	, .T. }, ; //X3_OBRIGAT
	{ 'Vazio() .Or. Pertence("ET")'											, .T. }, ; //X3_VLDUSER
	{ 'E=Embarcador;T=Transportador;'										, .T. }, ; //X3_CBOX
	{ ''																	, .T. }, ; //X3_CBOXSPA
	{ ''																	, .T. }, ; //X3_CBOXENG
	{ ''																	, .T. }, ; //X3_PICTVAR
	{ ''																	, .T. }, ; //X3_WHEN
...
//
aHlpPor := {}
aAdd( aHlpPor, 'Tipo de Averbação de Carga' )

PutSX1Help( "PA2_I_TPAVE", aHlpPor, {}, {}, .T.,,.T. )
AutoGrLog( "Atualizado o Help do campo " + "A2_I_TPAVE" )
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP47782" ) ) ) ;
