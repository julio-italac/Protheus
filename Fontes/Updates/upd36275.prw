...
/*/
//--------------------------------------------------------------------
User Function UPD36275( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UPD36275" )
				Else
					MsgStop( "Atualização não Realizada.", "UPD36275" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'SA3'																	, .T. }, ; //X3_ARQUIVO
	{ '9S'																	, .T. }, ; //X3_ORDEM
	{ 'A3_I_EMWF'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 250																	, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'E-mail WF.'															, .T. }, ; //X3_TITULO
	{ 'E-mail WF.'															, .T. }, ; //X3_TITSPA
	{ 'WF. e-mail'															, .T. }, ; //X3_TITENG
	{ 'E-mail para workflow'												, .T. }, ; //X3_DESCRIC
	{ 'E-mail de workflow'													, .T. }, ; //X3_DESCSPA
	{ 'Workflow e-mail'														, .T. }, ; //X3_DESCENG
	{ ''																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
aAdd( aSX3, { ;
	{ 'SA3'																	, .T. }, ; //X3_ARQUIVO
	{ '9T'																	, .T. }, ; //X3_ORDEM
	{ 'A3_I_CCWF'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 250																	, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Cc Worflow'															, .T. }, ; //X3_TITULO
	{ 'Cc Worflow'															, .T. }, ; //X3_TITSPA
	{ 'Wc Worflow'															, .T. }, ; //X3_TITENG
	{ 'Com copia worflow'													, .T. }, ; //X3_DESCRIC
	{ 'Com copia worflow'													, .T. }, ; //X3_DESCSPA
	{ 'With copies worflow'													, .T. }, ; //X3_DESCENG
	{ ''																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	{ 'x       x       x       x       x       x       x       x       x       x       x       x       x       x       x x', .T. }, ; //X3_USADO
	{ ''																	, .T. }, ; //X3_RELACAO
...
//
aHlpPor := {}
aAdd( aHlpPor, 'Destinado a colocar os emails da equipe' )
aAdd( aHlpPor, 'do vendedor para o workflow de títulos' )
aAdd( aHlpPor, 'atrasados.' )

PutSX1Help( "PA3_I_EMWF ", aHlpPor, {}, {}, .T.,,.T. )
AutoGrLog( "Atualizado o Help do campo " + "A3_I_EMWF" )

aHlpPor := {}
aAdd( aHlpPor, 'Destinado a colocar os funcionarios' )
aAdd( aHlpPor, 'internos para o workflow de titulos' )
aAdd( aHlpPor, 'atrasados' )

PutSX1Help( "PA3_I_CCWF ", aHlpPor, {}, {}, .T.,,.T. )
AutoGrLog( "Atualizado o Help do campo " + "A3_I_CCWF" )
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPD36275" ) ) ) ;
