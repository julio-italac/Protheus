...
/*/
//--------------------------------------------------------------------
User Function UP27870( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UP27870" )
				Else
					MsgStop( "Atualização não Realizada.", "UP27870" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'SZX'																	, .T. }, ; //X3_ARQUIVO
	{ '67'																	, .T. }, ; //X3_ORDEM
	{ 'ZX_I_IBOLE'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Imp boleto?'															, .T. }, ; //X3_TITULO
	{ 'Imp boleto?'															, .T. }, ; //X3_TITSPA
	{ 'Imp boleto?'															, .T. }, ; //X3_TITENG
	{ 'Imprime boleto?S=Sim,N=Na'											, .T. }, ; //X3_DESCRIC
	{ 'Imprime boleto?S=Sim,N=Na'											, .T. }, ; //X3_DESCSPA
	{ 'Imprime boleto?S=Sim,N=Na'											, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP27870" ) ) ) ;
