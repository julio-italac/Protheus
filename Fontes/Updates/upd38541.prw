...
/*/
//--------------------------------------------------------------------
User Function UPD38541( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UPD38541" )
				Else
					MsgStop( "Atualização não Realizada.", "UPD38541" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'SC7'																	, .T. }, ; //X3_ARQUIVO
	{ '38'																	, .T. }, ; //X3_ORDEM
	{ 'C7_I_OBSSC'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 100																	, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Obs.SC Gener'														, .T. }, ; //X3_TITULO
	{ 'Obs.SC Gener'														, .T. }, ; //X3_TITSPA
	{ 'Obs.SC Gener'														, .T. }, ; //X3_TITENG
	{ 'Obs. SC Generica'													, .T. }, ; //X3_DESCRIC
	{ 'Obs. SC Generica'													, .T. }, ; //X3_DESCSPA
	{ 'Obs. SC Generica'													, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	{ 'x       x       x       x       x       x       x       x       x       x       x       x       x       x       x x', .T. }, ; //X3_USADO
	{ 'IF(!INCLUI,Posicione("SC1",1,SC7->(C7_FILIAL+C7_NUMSC+C7_ITEMSC), "C1_I_OBSSC"),"")', .T. }, ; //X3_RELACAO
	{ ''																	, .T. }, ; //X3_F3
	{ 0																		, .T. }, ; //X3_NIVEL
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPD38541" ) ) ) ;
