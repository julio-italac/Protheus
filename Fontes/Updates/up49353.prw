...
/*/
//--------------------------------------------------------------------
User Function UP49353( cEmpAmb, cFilAmb )
Local   aSay      := {}
Local   aButton   := {}
Local   aMarcadas := {}
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgInfo( "Atualização realizada.", "UP49353" )
				Else
					MsgStop( "Atualização não realizada.", "UP49353" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'ZK1'																	, .T. }, ; //X3_ARQUIVO
	{ '39'																	, .T. }, ; //X3_ORDEM
	{ 'ZK1_AGUARD'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Aguardando?'															, .T. }, ; //X3_TITULO
	{ 'Aguardando?'															, .T. }, ; //X3_TITSPA
	{ 'Aguardando?'															, .T. }, ; //X3_TITENG
	{ 'Aguardando Aprovacao'												, .T. }, ; //X3_DESCRIC
	{ 'Aguardando Aprovacao'												, .T. }, ; //X3_DESCSPA
	{ 'Aguardando Aprovacao'												, .T. }, ; //X3_DESCENG
	{ ''																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP49353" ) ) ) ;
