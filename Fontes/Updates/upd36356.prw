...
/*/
//--------------------------------------------------------------------
User Function UPD36356( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UPD36356" )
				Else
					MsgStop( "Atualização não Realizada.", "UPD36356" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aHlpPor, 'iden-tificar a procedência dos títulos,' )
aAdd( aHlpPor, 'per-mitindo a consolidação por este' )
aAdd( aHlpPor, 'itemeo controle orçamentário.' )

PutSX1Help( "PZFC_NATUR ", aHlpPor, {}, {}, .T.,,.T. )
AutoGrLog( "Atualizado o Help do campo " + "ZFC_NATUR" )
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPD36356" ) ) ) ;
