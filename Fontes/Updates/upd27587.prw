...
/*/
//--------------------------------------------------------------------
User Function UPD27587( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UPD27587" )
				Else
					MsgStop( "Atualização não Realizada.", "UPD27587" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX2, { ;
	...
	''																		, ; //X2_ROTINA
	''																		, ; //X2_PYME
	'ZGH_FILIAL+ZGH_CLIENT+ZGH_CLILOJ+ZGH_FORNER+ZGH_FORLOJ+ZGH_GRUPO+ZGH_PRODUT+ZGH_CFOP+ZGH_TPMOV+ZGH_NATURE+ZGH_IMPOST+ZGH_ALIQ', ; //X2_UNICO
	''																		, ; //X2_DISPLAY
	''																		, ; //X2_SYSOBJ
	''																		, ; //X2_USROBJ
	''																		, ; //X2_POSLGT
...
aAdd( aSX3, { ;
	...
	{ 'Imposto'																, .T. }, ; //X3_TITSPA
	{ 'Imposto'																, .T. }, ; //X3_TITENG
	{ 'Imposto'																, .T. }, ; //X3_DESCRIC
	{ 'Imposto'																, .T. }, ; //X3_DESCSPA
	{ 'Imposto'																, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	...
	{ 'R'																	, .T. }, ; //X3_CONTEXT
	{ ''																	, .T. }, ; //X3_OBRIGAT
	{ "U_AFIS008Val('ALQ')"													, .T. }, ; //X3_VLDUSER
	{ '1=PIS;2=COFINS'														, .T. }, ; //X3_CBOX
	{ ''																	, .T. }, ; //X3_CBOXSPA
	{ ''																	, .T. }, ; //X3_CBOXENG
	{ ''																	, .T. }, ; //X3_PICTVAR
	{ ''																	, .T. }, ; //X3_WHEN
...
aAdd( aSX3, { ;
	...
	{ 7																		, .T. }, ; //X3_TAMANHO
	{ 4																		, .T. }, ; //X3_DECIMAL
	{ 'Aliquota'															, .T. }, ; //X3_TITULO
	{ 'Aliquota'															, .T. }, ; //X3_TITSPA
	{ 'Aliquota'															, .T. }, ; //X3_TITENG
	{ 'Aliquota'															, .T. }, ; //X3_DESCRIC
	{ 'Aliquota'															, .T. }, ; //X3_DESCSPA
	{ 'Aliquota'															, .T. }, ; //X3_DESCENG
	{ '@E 99.9999'															, .T. }, ; //X3_PICTURE
	{ 'Vazio().OR.Positivo()'												, .T. }, ; //X3_VALID
	{ Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) + ;
	Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) + ;
	Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(160)					, .T. }, ; //X3_USADO
	{ ''																	, .T. }, ; //X3_RELACAO
...
aAdd( aSIX, { ;
	'ZGH'																	, ; //INDICE
	'5'																		, ; //ORDEM
	'ZGH_FILIAL+ZGH_IMPOST+ZGH_ALIQ'										, ; //CHAVE
	'Imposto+Aliquota'														, ; //DESCRICAO
	''																		, ; //DESCSPA
	''																		, ; //DESCENG
	'U'																		, ; //PROPRI
	''																		, ; //F3
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPD27587" ) ) ) ;
