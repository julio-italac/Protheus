...
/*/
//--------------------------------------------------------------------
User Function UP30346( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UP30346" )
				Else
					MsgStop( "Atualização não Realizada.", "UP30346" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'SZX'																	, .T. }, ; //X3_ARQUIVO
	{ '69'																	, .T. }, ; //X3_ORDEM
	{ 'ZX_I_ENVML'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Enviou EMail'														, .T. }, ; //X3_TITULO
	{ 'Enviou EMail'														, .T. }, ; //X3_TITSPA
	{ 'Enviou EMail'														, .T. }, ; //X3_TITENG
	{ 'Enviou E-mail?'														, .T. }, ; //X3_DESCRIC
	{ 'Enviou E-mail?'														, .T. }, ; //X3_DESCSPA
	{ 'Enviou E-mail?'														, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP30346" ) ) ) ;
