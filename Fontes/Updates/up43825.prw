...
/*/
//--------------------------------------------------------------------
User Function UP43825( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UP43825" )
				Else
					MsgStop( "Atualização não Realizada.", "UP43825" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'SD3'																	, .T. }, ; //X3_ARQUIVO
	{ '9E'																	, .T. }, ; //X3_ORDEM
	{ 'D3_I_TPTRS'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Tp de Transf'														, .T. }, ; //X3_TITULO
	{ 'Tp de Transf'														, .T. }, ; //X3_TITSPA
	{ 'Tp de Transf'														, .T. }, ; //X3_TITENG
	{ 'Tipo de transferencia'												, .T. }, ; //X3_DESCRIC
	{ 'Tipo de transferencia'												, .T. }, ; //X3_DESCSPA
	{ 'Tipo de transferencia'												, .T. }, ; //X3_DESCENG
	{ ''																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	...
	{ 'R'																	, .T. }, ; //X3_CONTEXT
	{ ''																	, .T. }, ; //X3_OBRIGAT
	{ 'U_MT261VLD("D3_I_TPTRS") .And. (Vazio() .Or. Pertence("12345"))'		, .T. }, ; //X3_VLDUSER
	{ '1=550;2=551;3=553;4=554;5=555'										, .T. }, ; //X3_CBOX
	{ '1=550;2=551;3=553;4=554;5=555'										, .T. }, ; //X3_CBOXSPA
	{ '1=550;2=551;3=553;4=554;5=555'										, .T. }, ; //X3_CBOXENG
	{ ''																	, .T. }, ; //X3_PICTVAR
	{ 'U_IT_TPTRS()'														, .T. }, ; //X3_WHEN
	{ ''																	, .T. }, ; //X3_INIBRW
	{ ''																	, .T. }, ; //X3_GRPSXG
	{ ''																	, .T. }, ; //X3_FOLDER
	{ ''																	, .T. }, ; //X3_CONDSQL
...
aAdd( aSX3, { ;
	{ 'SD3'																	, .T. }, ; //X3_ARQUIVO
	{ 'A5'																	, .T. }, ; //X3_ORDEM
	{ 'D3_I_DSCTM'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 50																	, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Desc.Tp.Tran'														, .T. }, ; //X3_TITULO
	{ 'Desc.Tp.Tran'														, .T. }, ; //X3_TITSPA
	{ 'Desc.Tp.Tran'														, .T. }, ; //X3_TITENG
	{ 'Descricao Tipo Transferen'											, .T. }, ; //X3_DESCRIC
	{ 'Descricao Tipo Transferen'											, .T. }, ; //X3_DESCSPA
	{ 'Descricao Tipo Transferen'											, .T. }, ; //X3_DESCENG
	{ ''																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	{ 'x       x       x       x       x       x       x       x       x       x       x       x       x       x       x x', .T. }, ; //X3_USADO
	{ ''																	, .T. }, ; //X3_RELACAO
...
//
aHlpPor := {}
aAdd( aHlpPor, 'Descrição Tipo de Transferência.' )

PutSX1Help( "PD3_I_DSCTM", aHlpPor, {}, {}, .T.,,.T. )
AutoGrLog( "Atualizado o Help do campo " + "D3_I_DSCTM" )
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP43825" ) ) ) ;
