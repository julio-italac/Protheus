...
/*/
//--------------------------------------------------------------------
User Function UP48636( cEmpAmb, cFilAmb )
Local   aSay      := {}
Local   aButton   := {}
Local   aMarcadas := {}
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgInfo( "Atualização realizada.", "UP48636" )
				Else
					MsgStop( "Atualização não realizada.", "UP48636" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'ZEL'																	, .T. }, ; //X3_ARQUIVO
	{ '15'																	, .T. }, ; //X3_ORDEM
	{ 'ZEL_DIAUTI'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Sab.diaUtil?'														, .T. }, ; //X3_TITULO
	{ 'Sab.diaUtil?'														, .T. }, ; //X3_TITSPA
	{ 'Sab.diaUtil?'														, .T. }, ; //X3_TITENG
	{ 'Sabado conta como dia uti'											, .T. }, ; //X3_DESCRIC
	{ 'Sabado conta como dia uti'											, .T. }, ; //X3_DESCSPA
	{ 'Sabado conta como dia uti'											, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
...
aAdd( aSX3, { ;
	{ 'ZZL'																	, .T. }, ; //X3_ARQUIVO
	{ '9S'																	, .T. }, ; //X3_ORDEM
	{ 'ZZL_EXOCRD'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Exc.Oco.RDC?'														, .T. }, ; //X3_TITULO
	{ 'Exc.Oco.RDC?'														, .T. }, ; //X3_TITSPA
	{ 'Exc.Oco.RDC?'														, .T. }, ; //X3_TITENG
	{ 'Autoriza Exc. Ocor. RDC?'											, .T. }, ; //X3_DESCRIC
	{ 'Autoriza Exc. Ocor. RDC?'											, .T. }, ; //X3_DESCSPA
	{ 'Autoriza Exc. Ocor. RDC?'											, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	{ 'x       x       x       x       x       x       x       x       x       x       x       x       x       x       x x', .T. }, ; //X3_USADO
	{ "'N'"																	, .T. }, ; //X3_RELACAO
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP48636" ) ) ) ;
