...
/*/
//--------------------------------------------------------------------
User Function UP49427( cEmpAmb, cFilAmb )
Local   aSay      := {}
Local   aButton   := {}
Local   aMarcadas := {}
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização realizada.", "UP49427" )
				Else
					MsgStop( "Atualização não realizada.", "UP49427" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	...
	{ ''																	, .T. }, ; //X3_OBRIGAT
	{ ''																	, .T. }, ; //X3_VLDUSER
	{ 'N=NAO PROCESSADO;P=PROCESSADO;R=REJEITADO;O=OBSOLETO;A=CARGA ALTERADA'	, .T. }, ; //X3_CBOX
	{ 'N=NAO PROCESSADO;P=PROCESSADO;R=REJEITADO;O=OBSOLETO;A=CARGA ALTERADA'	, .T. }, ; //X3_CBOXSPA
	{ 'N=NAO PROCESSADO;P=PROCESSADO;R=REJEITADO;O=OBSOLETO;A=CARGA ALTERADA'	, .T. }, ; //X3_CBOXENG
	{ ''																	, .T. }, ; //X3_PICTVAR
	{ ''																	, .T. }, ; //X3_WHEN
	{ ''																	, .T. }, ; //X3_INIBRW
	{ ''																	, .T. }, ; //X3_GRPSXG
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP49427" ) ) ) ;
