...
/*/
//--------------------------------------------------------------------
User Function UP48652( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UP48652" )
				Else
					MsgStop( "Atualização não Realizada.", "UP48652" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'ZZL'																	, .T. }, ; //X3_ARQUIVO
	{ 'A2'																	, .T. }, ; //X3_ORDEM
	{ 'ZZL_TPADMR'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Tipo Adm E R'														, .T. }, ; //X3_TITULO
	{ 'Tipo Adm E R'														, .T. }, ; //X3_TITSPA
	{ 'Tipo Adm E R'														, .T. }, ; //X3_TITENG
	{ 'Tipo Adm.Eliminacao Resid'											, .T. }, ; //X3_DESCRIC
	{ 'Tipo Adm.Eliminacao Resid'											, .T. }, ; //X3_DESCSPA
	{ 'Tipo Adm.Eliminacao Resid'											, .T. }, ; //X3_DESCENG
	{ '9'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	...
	{ 'R'																	, .T. }, ; //X3_CONTEXT
	{ ''																	, .T. }, ; //X3_OBRIGAT
	{ 'Vazio() .Or. Pertence("12")'											, .T. }, ; //X3_VLDUSER
	{ '1=Compras;2=Suprimento do Leite'				        				, .T. }, ; //X3_CBOX
	{ ''																	, .T. }, ; //X3_CBOXSPA
	{ ''																	, .T. }, ; //X3_CBOXENG
	{ ''																	, .T. }, ; //X3_PICTVAR
	{ ''																	, .T. }, ; //X3_WHEN
...
//
aHlpPor := {}
aAdd( aHlpPor, 'Tipo Adm.Eliminação Residuo' )

PutSX1Help( "PZZL_TPADMR", aHlpPor, {}, {}, .T.,,.T. )
AutoGrLog( "Atualizado o Help do campo " + "ZZL_TPADMR" )
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP48652" ) ) ) ;
