...
/*/
//--------------------------------------------------------------------
User Function UP49604( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UP49604" )
				Else
					MsgStop( "Atualização não Realizada.", "UP49604" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'DAK'																	, .T. }, ; //X3_ARQUIVO
	{ '9G'																	, .T. }, ; //X3_ORDEM
	{ 'DAK_I_RATF'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Rateio Frete'														, .T. }, ; //X3_TITULO
	{ 'Rateio Frete'														, .T. }, ; //X3_TITSPA
	{ 'Rateio Frete'														, .T. }, ; //X3_TITENG
	{ 'Rateio de Frete'														, .T. }, ; //X3_DESCRIC
	{ 'Rateio de Frete'														, .T. }, ; //X3_DESCSPA
	{ 'Rateio de Frete'														, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	...
	{ 'R'																	, .T. }, ; //X3_CONTEXT
	{ ''																	, .T. }, ; //X3_OBRIGAT
	{ 'Vazio() .Or. Pertence("SNC")'										, .T. }, ; //X3_VLDUSER
	{ 'S=Sim;N=Nao;C=Cancelado'												, .T. }, ; //X3_CBOX
	{ ''																	, .T. }, ; //X3_CBOXSPA
	{ ''																	, .T. }, ; //X3_CBOXENG
	{ ''																	, .T. }, ; //X3_PICTVAR
	{ ''																	, .T. }, ; //X3_WHEN
...
//
aHlpPor := {}
aAdd( aHlpPor, 'Rateio de Frete' )

PutSX1Help( "PDAK_I_RATF", aHlpPor, {}, {}, .T.,,.T. )
AutoGrLog( "Atualizado o Help do campo " + "DAK_I_RATF" )
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP49604" ) ) ) ;
