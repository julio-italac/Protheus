...
/*/
//--------------------------------------------------------------------
User Function UP36404B( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UP36404B" )
				Else
					MsgStop( "Atualização não Realizada.", "UP36404B" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	...
	{ 'R'																	, .T. }, ; //X3_CONTEXT
	{ ''																	, .T. }, ; //X3_OBRIGAT
	{ 'U_AOMS073("C6_PRODUTO")'												, .T. }, ; //X3_VLDUSER
	{ ''																	, .T. }, ; //X3_CBOX
	{ ''																	, .T. }, ; //X3_CBOXSPA
	{ ''																	, .T. }, ; //X3_CBOXENG
	{ ''																	, .T. }, ; //X3_PICTVAR
...
aAdd( aSX3, { ;
	{ 'SC6'																	, .T. }, ; //X3_ARQUIVO
	{ 'C7'																	, .T. }, ; //X3_ORDEM
	{ 'C6_I_PTBRU'															, .T. }, ; //X3_CAMPO
	{ 'N'																	, .T. }, ; //X3_TIPO
	{ 15																	, .T. }, ; //X3_TAMANHO
	{ 4																		, .T. }, ; //X3_DECIMAL
	{ 'Peso Tot Bru'														, .T. }, ; //X3_TITULO
	{ 'Peso Tot Bru'														, .T. }, ; //X3_TITSPA
	{ 'Peso Tot Bru'														, .T. }, ; //X3_TITENG
	{ 'Peso Total Bruto'													, .T. }, ; //X3_DESCRIC
	{ 'Peso Total Bruto'													, .T. }, ; //X3_DESCSPA
	{ 'Peso Total Bruto'													, .T. }, ; //X3_DESCENG
	{ '@E 9,999,999,999.9999'												, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	{ 'x       x       x       x       x       x       x       x       x       x       x       x       x       x       x x', .T. }, ; //X3_USADO
	{ ''																	, .T. }, ; //X3_RELACAO
	{ ''																	, .T. }, ; //X3_F3
	...
	{ ''																	, .T. }, ; //X3_CBOXENG
	{ ''																	, .T. }, ; //X3_PICTVAR
	{ 'U_AOMS027W("C6_I_PTBRU")'											, .T. }, ; //X3_WHEN
	{ ''																	, .T. }, ; //X3_INIBRW
	{ ''																	, .T. }, ; //X3_GRPSXG
	{ ''																	, .T. }, ; //X3_FOLDER
	{ ''																	, .T. }, ; //X3_CONDSQL
...
aAdd( aSX7, { ;
	'C6_QTDVEN'																, ; //X7_CAMPO
	'002'																	, ; //X7_SEQUENC
	'U_AOMS027G("C6_QTDVEN")'												, ; //X7_REGRA
	'C6_I_PTBRU'															, ; //X7_CDOMIN
	'P'																		, ; //X7_TIPO
	'N'																		, ; //X7_SEEK
	''																		, ; //X7_ALIAS
	0																		, ; //X7_ORDEM
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP36404B" ) ) ) ;
