...
/*/
//--------------------------------------------------------------------
User Function UP48915A( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UP48915A" )
				Else
					MsgStop( "Atualização não Realizada.", "UP48915A" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'ZLJ'																	, .T. }, ; //X3_ARQUIVO
	{ '62'																	, .T. }, ; //X3_ORDEM
	{ 'ZLJ_ENVEVO'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Env.Col.EvoM'														, .T. }, ; //X3_TITULO
	{ 'Env.Col.EvoM'														, .T. }, ; //X3_TITSPA
	{ 'Env.Col.EvoM'														, .T. }, ; //X3_TITENG
	{ 'Envia Coletas P/Evomilk?'											, .T. }, ; //X3_DESCRIC
	{ 'Envia Coletas P/Evomilk?'											, .T. }, ; //X3_DESCSPA
	{ 'Envia Coletas P/Evomilk?'											, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
//
aHlpPor := {}
aAdd( aHlpPor, 'Envia Coletas para a Evomilk?' )

PutSX1Help( "PZLJ_ENVEVO", aHlpPor, {}, {}, .T.,,.T. )
AutoGrLog( "Atualizado o Help do campo " + "ZLJ_ENVEVO" )
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP48915A" ) ) ) ;
