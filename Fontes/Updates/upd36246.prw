...
/*/
//--------------------------------------------------------------------
User Function UPD36246( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UPD36246" )
				Else
					MsgStop( "Atualização não Realizada.", "UPD36246" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'SA3'																	, .T. }, ; //X3_ARQUIVO
	{ '30'																	, .T. }, ; //X3_ORDEM
	{ 'A3_I_INAT'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 6																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Vend. Inat.'															, .T. }, ; //X3_TITULO
	{ 'Vend. inact.'														, .T. }, ; //X3_TITSPA
	{ 'Seller Inact'														, .T. }, ; //X3_TITENG
	{ 'Vendedor Inativo'													, .T. }, ; //X3_DESCRIC
	{ 'Vendedor inactivo'													, .T. }, ; //X3_DESCSPA
	{ 'Seller Inactive'														, .T. }, ; //X3_DESCENG
	{ ''																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
//
aHlpPor := {}
aAdd( aHlpPor, 'No processo de inativação de' )
aAdd( aHlpPor, 'cliente,qdoesse registro referir-se a um' )
aAdd( aHlpPor, 'gerente,  este vendedor inativo será' )
aAdd( aHlpPor, 'atribuido aosClientes dos vendedores' )
aAdd( aHlpPor, 'deste gerente.' )

PutSX1Help( "PA3_I_INAT ", aHlpPor, {}, {}, .T.,,.T. )
AutoGrLog( "Atualizado o Help do campo " + "A3_I_INAT" )
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPD36246" ) ) ) ;
