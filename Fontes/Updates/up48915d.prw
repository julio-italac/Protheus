...
/*/
//--------------------------------------------------------------------
User Function UP48915D( cEmpAmb, cFilAmb )
Local   aSay      := {}
Local   aButton   := {}
Local   aMarcadas := {}
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgInfo( "Atualização realizada.", "UP48915D" )
				Else
					MsgStop( "Atualização não realizada.", "UP48915D" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'SF1'																	, .T. }, ; //X3_ARQUIVO
	{ 'D2'																	, .T. }, ; //X3_ORDEM
	{ 'F1_I_DTEEV'															, .T. }, ; //X3_CAMPO
	{ 'D'																	, .T. }, ; //X3_TIPO
	{ 8																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Dt.Env.Evomi'														, .T. }, ; //X3_TITULO
	{ 'Dt.Env.Evomi'														, .T. }, ; //X3_TITSPA
	{ 'Dt.Env.Evomi'														, .T. }, ; //X3_TITENG
	{ 'Data Envio Evomilk'													, .T. }, ; //X3_DESCRIC
	{ 'Data Envio Evomilk'													, .T. }, ; //X3_DESCSPA
	{ 'Data Envio Evomilk'													, .T. }, ; //X3_DESCENG
	{ ''																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
aAdd( aSX3, { ;
	{ 'SF1'																	, .T. }, ; //X3_ARQUIVO
	{ 'D3'																	, .T. }, ; //X3_ORDEM
	{ 'F1_I_HREEV'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 8																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Hr.Env.Evomi'														, .T. }, ; //X3_TITULO
	{ 'Hr.Env.Evomi'														, .T. }, ; //X3_TITSPA
	{ 'Hr.Env.Evomi'														, .T. }, ; //X3_TITENG
	{ 'Hora Envio Evomilk'													, .T. }, ; //X3_DESCRIC
	{ 'Hora Envio Evomilk'													, .T. }, ; //X3_DESCSPA
	{ 'Hora Envio Evomilk'													, .T. }, ; //X3_DESCENG
	{ ''																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	{ 'x       x       x       x       x       x       x       x       x       x       x       x       x       x       x x', .T. }, ; //X3_USADO
	{ ''																	, .T. }, ; //X3_RELACAO
...
//
aHlpPor := {}
aAdd( aHlpPor, 'Data Envio Evomilk' )

PutSX1Help( "PF1_I_DTEEV", aHlpPor, {}, {}, .T.,,.T. )
AutoGrLog( "Atualizado o Help do campo " + "F1_I_DTEEV" )

aHlpPor := {}
aAdd( aHlpPor, 'Hora Envio Evomilk' )

PutSX1Help( "PF1_I_HREEV", aHlpPor, {}, {}, .T.,,.T. )
AutoGrLog( "Atualizado o Help do campo " + "F1_I_HREEV" )
...
PutSX1Help( "PZBX_JSONEX", aHlpPor, {}, {}, .T.,,.T. )
AutoGrLog( "Atualizado o Help do campo " + "ZBX_JSONEX" )
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP48915D" ) ) ) ;
