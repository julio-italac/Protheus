...
/*/
//--------------------------------------------------------------------
User Function UP40619( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UP40619" )
				Else
					MsgStop( "Atualização não Realizada.", "UP40619" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'ZE2'																	, .T. }, ; //X3_ARQUIVO
	{ '14'																	, .T. }, ; //X3_ORDEM
	{ 'ZE2_DTCONT'															, .T. }, ; //X3_CAMPO
	{ 'D'																	, .T. }, ; //X3_TIPO
	{ 8																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Dt Contagem'															, .T. }, ; //X3_TITULO
	{ 'Dt Contagem'															, .T. }, ; //X3_TITSPA
	{ 'Dt Contagem'															, .T. }, ; //X3_TITENG
	{ 'Data da Contagem'													, .T. }, ; //X3_DESCRIC
	{ 'Data da Contagem'													, .T. }, ; //X3_DESCSPA
	{ 'Data da Contagem'													, .T. }, ; //X3_DESCENG
	{ ''																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	{ 'x       x       x       x       x       x       x       x       x       x       x       x       x       x       x x', .T. }, ; //X3_USADO
	{ 'DDATABASE - 1'														, .T. }, ; //X3_RELACAO
	{ ''																	, .T. }, ; //X3_F3
	{ 0																		, .T. }, ; //X3_NIVEL
	{ 'xxxxxx x'															, .T. }, ; //X3_RESERV
	{ ''																	, .T. }, ; //X3_CHECK
	...
	{ 'R'																	, .T. }, ; //X3_CONTEXT
	{ ''																	, .T. }, ; //X3_OBRIGAT
	{ 'U_AEST042VLD()'														, .T. }, ; //X3_VLDUSER
	{ ''																	, .T. }, ; //X3_CBOX
	{ ''																	, .T. }, ; //X3_CBOXSPA
	{ ''																	, .T. }, ; //X3_CBOXENG
	{ ''																	, .T. }, ; //X3_PICTVAR
	{ 'U_AEST042W()'														, .T. }, ; //X3_WHEN
	{ ''																	, .T. }, ; //X3_INIBRW
	{ ''																	, .T. }, ; //X3_GRPSXG
	{ ''																	, .T. }, ; //X3_FOLDER
	{ ''																	, .T. }, ; //X3_CONDSQL
...
aAdd( aSX3, { ;
	...
	{ 'Data Inclusao'														, .T. }, ; //X3_DESCRIC
	{ 'Data Inclusao'														, .T. }, ; //X3_DESCSPA
	{ 'Data Inclusao'														, .T. }, ; //X3_DESCENG
	{ ''																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	{ 'x       x       x       x       x       x       x       x       x       x       x       x       x       x       x x', .T. }, ; //X3_USADO
...
aAdd( aSIX, { ;
	'ZE2'																	, ; //INDICE
	'3'																		, ; //ORDEM
	'ZE2_FILIAL+ZE2_PRODUT+DTOS(ZE2_DTCONT)+ZE2_LOCAL'						, ; //CHAVE
	'Produto+Dt Contagem+Armazem'											, ; //DESCRICAO
	'Produto+Dt Contagem+Armazem'											, ; //DESCSPA
	'Produto+Dt Contagem+Armazem'											, ; //DESCENG
	'U'																		, ; //PROPRI
	''																		, ; //F3
	''																		, ; //NICKNAME
	'S'																		} ) //SHOWPESQ
...
aAdd( aSIX, { ;
	'ZE2'																	, ; //INDICE
	'4'																		, ; //ORDEM
	'ZE2_FILIAL+ZE2_PRODUT+DTOS(ZE2_DTINCL)+ZE2_LOCAL'						, ; //CHAVE
	'Produto+Dt Inclusao+Armazem'											, ; //DESCRICAO
	'Produto+Dt Inclusao+Armazem'											, ; //DESCSPA
	'Produto+Dt Inclusao+Armazem'											, ; //DESCENG
	'U'																		, ; //PROPRI
	''																		, ; //F3
	''																		, ; //NICKNAME
	'S'																		} ) //SHOWPESQ
...
PutSX1Help( "PZE2_LOCAL ", aHlpPor, {}, {}, .T.,,.T. )
AutoGrLog( "Atualizado o Help do campo " + "ZE2_LOCAL" )
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP40619" ) ) ) ;
