...
/*/
//--------------------------------------------------------------------
User Function UPD44571( cEmpAmb, cFilAmb )
Local   aSay      := {}
Local   aButton   := {}
Local   aMarcadas := {}
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização realizada.", "UPD44571" )
				Else
					MsgStop( "Atualização não realizada.", "UPD44571" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'SF2'																	, .T. }, ; //X3_ARQUIVO
	{ 'ZZ'																	, .T. }, ; //X3_ORDEM
	{ 'F2_I_DTOP'															, .T. }, ; //X3_CAMPO
	{ 'D'																	, .T. }, ; //X3_TIPO
	{ 8																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Ent.OL.Canho'														, .T. }, ; //X3_TITULO
	{ 'Ent.OL.Canho'														, .T. }, ; //X3_TITSPA
	{ 'Ent.OL.Canho'														, .T. }, ; //X3_TITENG
	{ 'Entrega no OL-Dt.Canhoto'											, .T. }, ; //X3_DESCRIC
	{ 'Entrega no OL-Dt.Canhoto'											, .T. }, ; //X3_DESCSPA
	{ 'Entrega no OL-Dt.Canhoto'											, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPD44571" ) ) ) ;
