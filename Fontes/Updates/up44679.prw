...
/*/
//--------------------------------------------------------------------
User Function UP44679( cEmpAmb, cFilAmb )
Local   aSay      := {}
Local   aButton   := {}
Local   aMarcadas := {}
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização realizada.", "UP44679" )
				Else
					MsgStop( "Atualização não realizada.", "UP44679" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	...
	2																		, ; //X3_TAMANHO
	0																		, ; //X3_DECIMAL
	'Dias Ad.Op.T'															, ; //X3_TITULO
	'Dias Ad.Op.T'															, ; //X3_TITSPA
	'Dias Ad.Op.T'															, ; //X3_TITENG
	'Dias Adicionais Oper.Tria'												, ; //X3_DESCRIC
	'Dias Adicionais Oper.Tria'												, ; //X3_DESCSPA
	'Dias Adicionais Oper.Tria'												, ; //X3_DESCENG
	'99'																	, ; //X3_PICTURE
	''																		, ; //X3_VALID
...
//
aHlpPor := {}
aAdd( aHlpPor, 'Dias Adicionais para Clientes Operação' )
aAdd( aHlpPor, 'Triangular.' )

aHlpEng := {}
aAdd( aHlpEng, 'Dias Adicionais para Clientes Operação' )
aAdd( aHlpEng, 'Triangular.' )

aHlpSpa := {}
aAdd( aHlpSpa, 'Dias Adicionais para Clientes Operação' )
aAdd( aHlpSpa, 'Triangular.' )

PutSX1Help( "PA1_I_ADDIA", aHlpPor, aHlpEng, aHlpSpa, .T.,,.T. )
AutoGrLog( "Atualizado o Help do campo " + "A1_I_ADDIA" )
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP44679" ) ) ) ;
