...
/*/
//--------------------------------------------------------------------
User Function UP48915C( cEmpAmb, cFilAmb )
Local   aSay      := {}
Local   aButton   := {}
Local   aMarcadas := {}
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgInfo( "Atualização realizada.", "UP48915C" )
				Else
					MsgStop( "Atualização não realizada.", "UP48915C" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	...
	20																		, ; //X3_TAMANHO
	0																		, ; //X3_DECIMAL
	'Dt.Hora Inte'															, ; //X3_TITULO
	'Dt.Hora Inte'															, ; //X3_TITSPA
	'Dt.Hora Inte'															, ; //X3_TITENG
	'Data Hora de Integracao'												, ; //X3_DESCRIC
	'Data Hora de Integracao'												, ; //X3_DESCSPA
	'Data Hora de Integracao'												, ; //X3_DESCENG
	''																		, ; //X3_PICTURE
	''																		, ; //X3_VALID
...
aAdd( aSIX, { ;
	'ZBX'																	, ; //INDICE
	'4'																		, ; //ORDEM
	'ZBX_FILIAL+ZBX_NRNFE+ZBX_SERNFE+ZBX_CODPRO+ZBX_LOJPRO+ZBX_STATUS'		, ; //CHAVE
	'Nota Fiscal+Serie+Cod Produtor+Loja Produtor+Status'					, ; //DESCRICAO
	'Nota Fiscal+Serie+Cod Produtor+Loja Produtor+Status'					, ; //DESCSPA
	'Nota Fiscal+Serie+Cod Produtor+Loja Produtor+Status'					, ; //DESCENG
	'U'																		, ; //PROPRI
	''																		, ; //F3
	''																		, ; //NICKNAME
	'S'																		} ) //SHOWPESQ
...
//
aHlpPor := {}
aAdd( aHlpPor, 'Data Hora de Integração' )

aHlpEng := {}
aAdd( aHlpEng, 'Data Hora de Integração' )

aHlpSpa := {}
aAdd( aHlpSpa, 'Data Hora de Integração' )

PutSX1Help( "PZBX_DTHORA", aHlpPor, aHlpEng, aHlpSpa, .T.,,.T. )
AutoGrLog( "Atualizado o Help do campo " + "ZBX_DTHORA" )

aHlpPor := {}
aAdd( aHlpPor, 'Observação' )

aHlpEng := {}
aAdd( aHlpEng, 'Observação' )

aHlpSpa := {}
aAdd( aHlpSpa, 'Observação' )

PutSX1Help( "PZBX_OBSERV", aHlpPor, aHlpEng, aHlpSpa, .T.,,.T. )
AutoGrLog( "Atualizado o Help do campo " + "ZBX_OBSERV" )
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP48915C" ) ) ) ;
