...
/*/
//--------------------------------------------------------------------
User Function UP46599B( cEmpAmb, cFilAmb )
Local   aSay      := {}
Local   aButton   := {}
Local   aMarcadas := {}
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgInfo( "Atualização realizada.", "UP46599B" )
				Else
					MsgStop( "Atualização não realizada.", "UP46599B" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'SB2'																	, .T. }, ; //X3_ARQUIVO
	{ 'ZZ'																	, .T. }, ; //X3_ORDEM
	{ 'B2_I_QLIBE'															, .T. }, ; //X3_CAMPO
	{ 'N'																	, .T. }, ; //X3_TIPO
	{ 18																	, .T. }, ; //X3_TAMANHO
	{ 4																		, .T. }, ; //X3_DECIMAL
	{ 'Saldo Manual'														, .T. }, ; //X3_TITULO
	{ 'Saldo Manual'														, .T. }, ; //X3_TITSPA
	{ 'Saldo Manual'														, .T. }, ; //X3_TITENG
	{ 'Saldo Manual'														, .T. }, ; //X3_DESCRIC
	{ 'Saldo Manual'														, .T. }, ; //X3_DESCSPA
	{ 'Saldo Manual'														, .T. }, ; //X3_DESCENG
	{ '@E 9,999,999,999,999.9999'											, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
aAdd( aSX3, { ;
	{ 'SC5'																	, .T. }, ; //X3_ARQUIVO
	{ 'ZZ'																	, .T. }, ; //X3_ORDEM
	{ 'C5_I_AGRUP'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 6																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Grupo Carga'															, .T. }, ; //X3_TITULO
	{ 'Grupo Carga'															, .T. }, ; //X3_TITSPA
	{ 'Grupo Carga'															, .T. }, ; //X3_TITENG
	{ 'Grupo de PVs de Carga'												, .T. }, ; //X3_DESCRIC
	{ 'Grupo de PVs de Carga'												, .T. }, ; //X3_DESCSPA
	{ 'Grupo de PVs de Carga'												, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	{ 'x       x       x       x       x       x       x       x       x       x       x       x       x       x       x x', .T. }, ; //X3_USADO
	{ ''																	, .T. }, ; //X3_RELACAO
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP46599B" ) ) ) ;
