...
/*/
//--------------------------------------------------------------------
User Function UP22140B( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UP22140B" )
				Else
					MsgStop( "Atualização não Realizada.", "UP22140B" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'TLW'																	, .T. }, ; //X3_ARQUIVO
	{ '10'																	, .T. }, ; //X3_ORDEM
	{ 'TLW_HRDEVO'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 5																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Hora Dev.'															, .T. }, ; //X3_TITULO
	{ 'Hora Dev.'															, .T. }, ; //X3_TITSPA
	{ 'Return time'															, .T. }, ; //X3_TITENG
	{ 'Hr Devolucao'														, .T. }, ; //X3_DESCRIC
	{ 'Hr.Devolucion'														, .T. }, ; //X3_DESCSPA
	{ 'Return Time'															, .T. }, ; //X3_DESCENG
	{ '99:99'																, .T. }, ; //X3_PICTURE
	{ 'MDTTLWDTHO(2)'														, .T. }, ; //X3_VALID
...
aAdd( aSX3, { ;
	{ 'TNF'																	, .T. }, ; //X3_ARQUIVO
	{ '39'																	, .T. }, ; //X3_ORDEM
	{ 'TNF_I_QTDD'															, .T. }, ; //X3_CAMPO
	{ 'N'																	, .T. }, ; //X3_TIPO
	{ 6																		, .T. }, ; //X3_TAMANHO
	{ 2																		, .T. }, ; //X3_DECIMAL
	{ 'Qtd.Dev.Parc'														, .T. }, ; //X3_TITULO
	{ 'Qtd.Dev.Parc'														, .T. }, ; //X3_TITSPA
	{ 'Qtd.Dev.Parc'														, .T. }, ; //X3_TITENG
	{ 'Quntidade Devolucao Parci'											, .T. }, ; //X3_DESCRIC
	{ 'Quntidade Devolucao Parci'											, .T. }, ; //X3_DESCSPA
	{ 'Quntidade Devolucao Parci'											, .T. }, ; //X3_DESCENG
...
aAdd( aSX3, { ;
	{ 'TNF'																	, .T. }, ; //X3_ARQUIVO
	{ '40'																	, .T. }, ; //X3_ORDEM
	{ 'TNF_I_IMPR'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Impr.Recibo'															, .T. }, ; //X3_TITULO
	{ 'Impr.Recibo'															, .T. }, ; //X3_TITSPA
	{ 'Impr.Recibo'															, .T. }, ; //X3_TITENG
	{ 'Imprime Recibo Devolucao'											, .T. }, ; //X3_DESCRIC
	{ 'Imprime Recibo Devolucao'											, .T. }, ; //X3_DESCSPA
	{ 'Imprime Recibo Devolucao'											, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	{ Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) + ;
	Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) + ;
...
aAdd( aSX7, { ;
	'TNF_QTDENT'															, ; //X7_CAMPO
	'001'																	, ; //X7_SEQUENC
	'M->TNF_QTDENT'															, ; //X7_REGRA
	'TNF_I_SLDD'															, ; //X7_CDOMIN
	'P'																		, ; //X7_TIPO
	'N'																		, ; //X7_SEEK
	''																		, ; //X7_ALIAS
	0																		, ; //X7_ORDEM
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP22140B" ) ) ) ;
