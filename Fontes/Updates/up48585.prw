...
/*/
//--------------------------------------------------------------------
User Function UP48585( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UP48585" )
				Else
					MsgStop( "Atualização não Realizada.", "UP48585" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'SB5'																	, .T. }, ; //X3_ARQUIVO
	{ 'DQ'																	, .T. }, ; //X3_ORDEM
	{ 'B5_I_DQUAR'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 2																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Dias Quarent'														, .T. }, ; //X3_TITULO
	{ 'Dias Quarent'														, .T. }, ; //X3_TITSPA
	{ 'Dias Quarent'														, .T. }, ; //X3_TITENG
	{ 'Nr. de Dias de Quarentena'											, .T. }, ; //X3_DESCRIC
	{ 'Nr. de Dias de Quarentena'											, .T. }, ; //X3_DESCSPA
	{ 'Nr. de Dias de Quarentena'											, .T. }, ; //X3_DESCENG
	{ '99'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
aAdd( aSX3, { ;
	{ 'SD7'																	, .T. }, ; //X3_ARQUIVO
	{ '43'																	, .T. }, ; //X3_ORDEM
	{ 'D7_I_DTPRE'															, .T. }, ; //X3_CAMPO
	{ 'D'																	, .T. }, ; //X3_TIPO
	{ 8																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Dt.Prev.Libe'														, .T. }, ; //X3_TITULO
	{ 'Dt.Prev.Libe'														, .T. }, ; //X3_TITSPA
	{ 'Dt.Prev.Libe'														, .T. }, ; //X3_TITENG
	{ 'Data Prevista Liberacao Q'											, .T. }, ; //X3_DESCRIC
	{ 'Data Prevista Liberacao Q'											, .T. }, ; //X3_DESCSPA
	{ 'Data Prevista Liberacao Q'											, .T. }, ; //X3_DESCENG
	{ ''																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	{ 'x       x       x       x       x       x       x       x       x       x       x       x       x       x       x x', .T. }, ; //X3_USADO
	{ ''																	, .T. }, ; //X3_RELACAO
...
aAdd( aSX7, { ;
	'D7_TIPO'																, ; //X7_CAMPO
	'502'																	, ; //X7_SEQUENC
	'IF(M->D7_TIPO=2,"39",SBZ->BZ_LOCPAD)'									, ; //X7_REGRA  // Nova Regra: IF(M->D7_TIPO=2,"39",SBZ->BZ_LOCPAD)   // Antiga regra: 'IF(M->D7_TIPO=2,SBZ->BZ_LOCPAD,"39")'	
	'D7_LOCDEST'															, ; //X7_CDOMIN
	'P'																		, ; //X7_TIPO
	'S'																		, ; //X7_SEEK
	'SBZ'																	, ; //X7_ALIAS
	1																		, ; //X7_ORDEM
	'xFilial("SD7")+SB1->B1_COD'											, ; //X7_CHAVE
	'U'																		, ; //X7_PROPRI
	''																		} ) //X7_CONDIC
...
//
aHlpPor := {}
aAdd( aHlpPor, 'Numero de dias de quarentena do produto.' )

PutSX1Help( "PB5_I_DQUAR", aHlpPor, {}, {}, .T.,,.T. )
AutoGrLog( "Atualizado o Help do campo " + "B5_I_DQUAR" )

//
// Helps Tabela SD7
//
aHlpPor := {}
aAdd( aHlpPor, 'Este campo podera ser preenchido com:' )
aAdd( aHlpPor, '1 - Liberar item do CQ' )
aAdd( aHlpPor, '2 - Rejeitar Item do CQ' )
aAdd( aHlpPor, '5 - Alterar Data de Validade do Lote' )

PutSX1Help( "PD7_TIPO   ", aHlpPor, {}, {}, .T.,,.T. )
AutoGrLog( "Atualizado o Help do campo " + "D7_TIPO" )

aHlpPor := {}
aAdd( aHlpPor, 'Tipos controlados pelo Sistema:' )
aAdd( aHlpPor, '0 - Saldo Inicial' )
aAdd( aHlpPor, '6 - Estorno da Liberacao' )
aAdd( aHlpPor, '7 - Estorno da Rejeicao' )
aAdd( aHlpPor, '8 - Despesas Agregadas' )


PutSX1Help( "SD7_TIPO   ", aHlpPor, {}, {}, .T.,,.T. )
AutoGrLog( "Atualizado a solução do campo " + "D7_TIPO" )

aHlpPor := {}
aAdd( aHlpPor, 'Data prevista de liberação do produto' )
aAdd( aHlpPor, 'daquarentena.' )

PutSX1Help( "PD7_I_DTPRE", aHlpPor, {}, {}, .T.,,.T. )
AutoGrLog( "Atualizado o Help do campo " + "D7_I_DTPRE" )
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP48585" ) ) ) ;
