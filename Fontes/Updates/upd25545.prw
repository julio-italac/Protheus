...
/*/
//--------------------------------------------------------------------
User Function UPD25545( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UPD25545" )
				Else
					MsgStop( "Atualização não Realizada.", "UPD25545" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'ZZL'																	, .T. }, ; //X3_ARQUIVO
	{ 'A8'																	, .T. }, ; //X3_ORDEM
	{ 'ZZL_DESFAZ'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Desfaz Vir.?'														, .T. }, ; //X3_TITULO
	{ 'Desfaz Vir.?'														, .T. }, ; //X3_TITSPA
	{ 'Desfaz Vir.?'														, .T. }, ; //X3_TITENG
	{ 'Desfazer virada de saldo?'											, .T. }, ; //X3_DESCRIC
	{ 'Desfazer virada de saldo?'											, .T. }, ; //X3_DESCSPA
	{ 'Desfazer virada de saldo?'											, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPD25545" ) ) ) ;
