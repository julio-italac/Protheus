...
/*/
//--------------------------------------------------------------------
User Function UPD38037( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UPD38037" )
				Else
					MsgStop( "Atualização não Realizada.", "UPD38037" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'SF1'																	, .T. }, ; //X3_ARQUIVO
	{ 'CP'																	, .T. }, ; //X3_ORDEM
	{ 'F1_I_FUNDF'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Fundo Fixo ?'														, .T. }, ; //X3_TITULO
	{ 'Fundo Fixo ?'														, .T. }, ; //X3_TITSPA
	{ 'Fundo Fixo ?'														, .T. }, ; //X3_TITENG
	{ 'Fundo Fixo ?'														, .T. }, ; //X3_DESCRIC
	{ 'Fundo Fixo ?'														, .T. }, ; //X3_DESCSPA
	{ 'Fundo Fixo ?'														, .T. }, ; //X3_DESCENG
	{ ''																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPD38037" ) ) ) ;
