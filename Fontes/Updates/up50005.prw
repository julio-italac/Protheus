...
/*/
//--------------------------------------------------------------------
User Function UP50005( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UP50005" )
				Else
					MsgStop( "Atualização não Realizada.", "UP50005" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	...
	{ 'Mostra status da SA'													, .T. }, ; //X3_DESCRIC
	{ 'Mostra status da SA'													, .T. }, ; //X3_DESCSPA
	{ 'Mostra status da SA'													, .T. }, ; //X3_DESCENG
	{ ''																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
//
aHlpPor := {}
aAdd( aHlpPor, 'Caso o item esteja com uma S.A aberta,' )
aAdd( aHlpPor, 'esse campo estará preenchido.' )

PutSX1Help( "PTNF_I_OBS ", aHlpPor, {}, {}, .T.,,.T. )
AutoGrLog( "Atualizado o Help do campo " + "TNF_I_OBS" )
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP50005" ) ) ) ;
