...
/*/
//--------------------------------------------------------------------
User Function UPD20782( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UPD20782" )
				Else
					MsgStop( "Atualização não Realizada.", "UPD20782" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	...
	{ 'Data Entrega'														, .F. }, ; //X3_DESCRIC
	{ 'Data Entrega'														, .F. }, ; //X3_DESCSPA
	{ 'Data Entrega'														, .F. }, ; //X3_DESCENG
	{ ''																	, .F. }, ; //X3_PICTURE
	{ ''																	, .F. }, ; //X3_VALID
...
aAdd( aSX3, { ;
	{ 'SC5'																	, .F. }, ; //X3_ARQUIVO
	{ 'F8'																	, .F. }, ; //X3_ORDEM
	{ 'C5_FECENT'															, .F. }, ; //X3_CAMPO
	{ 'D'																	, .F. }, ; //X3_TIPO
	{ 8																		, .F. }, ; //X3_TAMANHO
	{ 0																		, .F. }, ; //X3_DECIMAL
	{ 'Dt. Entrega'															, .F. }, ; //X3_TITULO
	{ 'Fch. Entrega'														, .F. }, ; //X3_TITSPA
	{ 'Del.Date'															, .F. }, ; //X3_TITENG
	{ 'Data de Entrega'														, .F. }, ; //X3_DESCRIC
	{ 'Fecha de Entrega'													, .F. }, ; //X3_DESCSPA
	{ 'Delivery Date'														, .F. }, ; //X3_DESCENG
	{ ''																	, .F. }, ; //X3_PICTURE
	{ ''																	, .F. }, ; //X3_VALID
	{ Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) + ;
	Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) + ;
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPD20782" ) ) ) ;
