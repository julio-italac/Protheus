...
/*/
//--------------------------------------------------------------------
User Function UP45105( cEmpAmb, cFilAmb )
Local   aSay      := {}
Local   aButton   := {}
Local   aMarcadas := {}
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização realizada.", "UP45105" )
				Else
					MsgStop( "Atualização não realizada.", "UP45105" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	...
	1																		, ; //X3_TAMANHO
	0																		, ; //X3_DECIMAL
	'Provisionado'															, ; //X3_TITULO
	'Provisionado'															, ; //X3_TITSPA
	'Provisionado'															, ; //X3_TITENG
	'Acordo Provisionado'													, ; //X3_DESCRIC
	'Acordo Provisionado'													, ; //X3_DESCSPA
	'Acordo Provisionado'													, ; //X3_DESCENG
	'@!'																	, ; //X3_PICTURE
	''																		, ; //X3_VALID
...
//
aHlpPor := {}
aAdd( aHlpPor, 'Informe se o acordo comercial é' )
aAdd( aHlpPor, 'provisionado: Sim/Não.' )

aHlpEng := {}
aAdd( aHlpEng, 'Informe se o acordo comercial é' )
aAdd( aHlpEng, 'provisionado: Sim/Não.' )

aHlpSpa := {}
aAdd( aHlpSpa, 'Informe se o acordo comercial é' )
aAdd( aHlpSpa, 'provisionado: Sim/Não.' )

PutSX1Help( "PZK1_PROV  ", aHlpPor, aHlpEng, aHlpSpa, .T.,,.T. )
AutoGrLog( "Atualizado o Help do campo " + "ZK1_PROV" )
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP45105" ) ) ) ;
