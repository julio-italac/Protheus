...
/*/
//--------------------------------------------------------------------
User Function UPD44782( cEmpAmb, cFilAmb )
Local   aSay      := {}
Local   aButton   := {}
Local   aMarcadas := {}
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgInfo( "Atualização realizada.", "UPD44782" )
				Else
					MsgStop( "Atualização não realizada.", "UPD44782" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'SC5'																	, .T. }, ; //X3_ARQUIVO
	{ '12'																	, .T. }, ; //X3_ORDEM
	{ 'C5_I_AGEND'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Tp Entrega'															, .T. }, ; //X3_TITULO
	{ 'Tp Entrega'															, .T. }, ; //X3_TITSPA
	{ 'Tp Entrega'															, .T. }, ; //X3_TITENG
	{ 'Tipo de Entrega'														, .T. }, ; //X3_DESCRIC
	{ 'Tipo de Entrega'														, .T. }, ; //X3_DESCSPA
	{ 'Tipo de Entrega'														, .T. }, ; //X3_DESCENG
	{ ''																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	...
	{ 'R'																	, .T. }, ; //X3_CONTEXT
	{ '  x'																	, .T. }, ; //X3_OBRIGAT
	{ 'IF(FindFunction("U_TipoEntrega"),U_TipoEntrega(M->C5_I_AGEND,.T.),.T.)'	, .T. }, ; //X3_VLDUSER
	{ ''																	, .T. }, ; //X3_CBOX
	{ ''																	, .T. }, ; //X3_CBOXSPA
	{ ''																	, .T. }, ; //X3_CBOXENG
	{ ''																	, .T. }, ; //X3_PICTVAR
	{ "INCLUI .OR. (ALTERA .AND. U_ITACSUSR('ZZL_PEDPOR','S'))"				, .T. }, ; //X3_WHEN
	{ ''																	, .T. }, ; //X3_INIBRW
	{ ''																	, .T. }, ; //X3_GRPSXG
	{ '1'																	, .T. }, ; //X3_FOLDER
	{ ''																	, .T. }, ; //X3_CONDSQL
...
aAdd( aSX3, { ;
	...
	{ 'Tipo de Venda'														, .T. }, ; //X3_DESCSPA
	{ 'Tipo de Venda'														, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	{ 'x       x       x       x       x       x       x       x       x       x       x       x       x       x       x x', .T. }, ; //X3_USADO
	{ '"F"'																	, .T. }, ; //X3_RELACAO
	...
	{ 'R'																	, .T. }, ; //X3_CONTEXT
	{ '  x'																	, .T. }, ; //X3_OBRIGAT
	{ 'Pertence("FV ")'														, .T. }, ; //X3_VLDUSER
	{ 'F=Carga Fechada;V=Fracionada'										, .T. }, ; //X3_CBOX
	{ ''																	, .T. }, ; //X3_CBOXSPA
	{ ''																	, .T. }, ; //X3_CBOXENG
	{ ''																	, .T. }, ; //X3_PICTVAR
	{ ''																	, .T. }, ; //X3_WHEN
...
aAdd( aSX7, { ;
	'C5_I_AGEND'															, ; //X7_CAMPO
	'502'																	, ; //X7_SEQUENC
	'DATE()+U_OMSVLDENT(,C5_CLIENTE,C5_LOJACLI,XFILIAL("SC5"),C5_NUM,1,,,C5_I_OPER,C5_I_TPVEN)', ; //X7_REGRA
	...
	''																		, ; //X7_CHAVE
	'U'																		, ; //X7_PROPRI
	"M->C5_I_AGEND $ 'I,O' .and. !l410Auto"									} ) //X7_CONDIC
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPD44782" ) ) ) ;
