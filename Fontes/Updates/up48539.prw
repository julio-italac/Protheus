...
==============================================================================================================================================================
André    - Julio Paz     - 16/09/24 - 17/01/25 -  48539  - Criação de campo para recalcular e alterar as quantidades dos itens de pedidos de compras do tipo serviço, conforme regras definidas.
==============================================================================================================================================================
*/
...
/*/
//--------------------------------------------------------------------
User Function UP48539( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UP48539" )
				Else
					MsgStop( "Atualização não Realizada.", "UP48539" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'SC1'																	, .T. }, ; //X3_ARQUIVO
	{ 'AY'																	, .T. }, ; //X3_ORDEM
	{ 'C1_I_SVPAR'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Crt.Ent.Parc'														, .T. }, ; //X3_TITULO
	{ 'Crt.Ent.Parc'														, .T. }, ; //X3_TITSPA
	{ 'Crt.Ent.Parc'														, .T. }, ; //X3_TITENG
	{ 'Controla Entrega Parcial?'											, .T. }, ; //X3_DESCRIC
	{ 'Controla Entrega Parcial?'											, .T. }, ; //X3_DESCSPA
	{ 'Controla Entrega Parcial?'											, .T. }, ; //X3_DESCENG
	{ ''																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
//
aHlpPor := {}
aAdd( aHlpPor, 'Controla Entrega Parcial?' )

PutSX1Help( "PC1_I_SVPAR", aHlpPor, {}, {}, .T.,,.T. )
AutoGrLog( "Atualizado o Help do campo " + "C1_I_SVPAR" )
...
PutSX1Help( "PC7_I_SVPAR", aHlpPor, {}, {}, .T.,,.T. )
AutoGrLog( "Atualizado o Help do campo " + "C7_I_SVPAR" )
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP48539" ) ) ) ;
