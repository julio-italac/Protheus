...
/*/
//--------------------------------------------------------------------
User Function UP47107( cEmpAmb, cFilAmb )
Local   aSay      := {}
Local   aButton   := {}
Local   aMarcadas := {}
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgInfo( "Atualização realizada.", "UP47107" )
				Else
					MsgStop( "Atualização não realizada.", "UP47107" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	...
	{ 'R'																	, .T. }, ; //X3_CONTEXT
	{ ''																	, .T. }, ; //X3_OBRIGAT
	{ 'Pertence("CONPQR")'													, .T. }, ; //X3_VLDUSER
	{ 'C=Atu.Dt.Entr Cli.;O=Atu.Dt.Entr.Op.Log.;N=Nao atualiza Dt.Entr.;P=Alt.PrevEnt Cli. e OL;Q=Chegada Op.Log.;R=Chegada Cli.', .T. }, ; //X3_CBOX
	{ 'C=Atu.Dt.Entr Cli.;O=Atu.Dt.Entr.Op.Log.;N=Nao atualiza Dt.Entr.;P=Alt.PrevEnt Cli. e OL;Q=Chegada Op.Log.;R=Chegada Cli.', .T. }, ; //X3_CBOXSPA
	{ 'C=Atu.Dt.Entr Cli.;O=Atu.Dt.Entr.Op.Log.;N=Nao atualiza Dt.Entr.;P=Alt.PrevEnt Cli. e OL;Q=Chegada Op.Log.;R=Chegada Cli.', .T. }, ; //X3_CBOXENG
	{ ''																	, .T. }, ; //X3_PICTVAR
	{ ''																	, .T. }, ; //X3_WHEN
	{ ''																	, .T. }, ; //X3_INIBRW
	{ ''																	, .T. }, ; //X3_GRPSXG
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP47107" ) ) ) ;
