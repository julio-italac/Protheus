...
/*/
//--------------------------------------------------------------------
User Function UPD44779( cEmpAmb, cFilAmb )
Local   aSay      := {}
Local   aButton   := {}
Local   aMarcadas := {}
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização realizada.", "UPD44779" )
				Else
					MsgStop( "Atualização não realizada.", "UPD44779" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'DAK'																	, .T. }, ; //X3_ARQUIVO
	{ '45'																	, .T. }, ; //X3_ORDEM
	{ 'DAK_I_ENV'															, .T. }, ; //X3_CAMPO
	{ 'N'																	, .T. }, ; //X3_TIPO
	{ 2																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Qtd Envio'															, .T. }, ; //X3_TITULO
	{ 'Qtd Envio'															, .T. }, ; //X3_TITSPA
	{ 'Qtd Envio'															, .T. }, ; //X3_TITENG
	{ 'Qtd Envios para Expedição'											, .T. }, ; //X3_DESCRIC
	{ 'Qtd Envios para Expedição'											, .T. }, ; //X3_DESCSPA
	{ 'Qtd Envios para Expedição'											, .T. }, ; //X3_DESCENG
	{ '99'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPD44779" ) ) ) ;
