...
/*/
//--------------------------------------------------------------------
User Function UP41123( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UP41123" )
				Else
					MsgStop( "Atualização não Realizada.", "UP41123" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'ZZL'																	, .T. }, ; //X3_ARQUIVO
	{ '9U'																	, .T. }, ; //X3_ORDEM
	{ 'ZZL_IPVTRA'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Inc/Alt.Pv.T'														, .T. }, ; //X3_TITULO
	{ 'Inc/Alt.Pv.T'														, .T. }, ; //X3_TITSPA
	{ 'Inc/Alt.Pv.T'														, .T. }, ; //X3_TITENG
	{ 'Inclui/Altera Pv.Transfer'											, .T. }, ; //X3_DESCRIC
	{ 'Inclui/Altera Pv.Transfer'											, .T. }, ; //X3_DESCSPA
	{ 'Inclui/Altera Pv.Transfer'											, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
//
aHlpPor := {}
aAdd( aHlpPor, 'Inclui, Altera, Exclui Pedidos de' )
aAdd( aHlpPor, 'Vendasde Transferência?' )

PutSX1Help( "PZZL_IPVTRA", aHlpPor, {}, {}, .T.,,.T. )
AutoGrLog( "Atualizado o Help do campo " + "ZZL_IPVTRA" )
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP41123" ) ) ) ;
