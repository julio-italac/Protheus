...
/*/
//--------------------------------------------------------------------
User Function UP46726( cEmpAmb, cFilAmb )
Local   aSay      := {}
Local   aButton   := {}
Local   aMarcadas := {}
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgInfo( "Atualização realizada.", "UP46726" )
				Else
					MsgStop( "Atualização não realizada.", "UP46726" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'SF2'																	, .T. }, ; //X3_ARQUIVO
	{ 'EV'																	, .T. }, ; //X3_ORDEM
	{ 'F2_I_DCIPI'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Inclui IPI'															, .T. }, ; //X3_TITULO
	{ 'Inclui IPI'															, .T. }, ; //X3_TITSPA
	{ 'Inclui IPI'															, .T. }, ; //X3_TITENG
	{ 'Inclui IPI'															, .T. }, ; //X3_DESCRIC
	{ 'Inclui IPI'															, .T. }, ; //X3_DESCSPA
	{ 'Inclui IPI'															, .T. }, ; //X3_DESCENG
	{ ''																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
aAdd( aSX3, { ;
	{ 'ZAZ'																	, .T. }, ; //X3_ARQUIVO
	{ '19'																	, .T. }, ; //X3_ORDEM
	{ 'ZAZ_IPIBAS'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Ipi na Base?'														, .T. }, ; //X3_TITULO
	{ 'Ipi na Base?'														, .T. }, ; //X3_TITSPA
	{ 'Ipi na Base?'														, .T. }, ; //X3_TITENG
	{ 'Ipi na Base?'														, .T. }, ; //X3_DESCRIC
	{ 'Ipi na Base?'														, .T. }, ; //X3_DESCSPA
	{ 'Ipi na Base?'														, .T. }, ; //X3_DESCENG
	{ ''																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	{ 'x       x       x       x       x       x       x       x       x       x       x       x       x       x       x x', .T. }, ; //X3_USADO
	{ '"S"'																	, .T. }, ; //X3_RELACAO
...
//
aHlpPor := {}
aAdd( aHlpPor, 'Ipi na Base?' )

PutSX1Help( "PZAZ_IPIBAS", aHlpPor, {}, {}, .T.,,.T. )
AutoGrLog( "Atualizado o Help do campo " + "ZAZ_IPIBAS" )
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP46726" ) ) ) ;
