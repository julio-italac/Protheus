...
/*/
//--------------------------------------------------------------------
User Function UP39908C( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UP39908C" )
				Else
					MsgStop( "Atualização não Realizada.", "UP39908C" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'SA1'																	, .T. }, ; //X3_ARQUIVO
	{ 'G3'																	, .T. }, ; //X3_ORDEM
	{ 'A1_I_OBSFO'															, .T. }, ; //X3_CAMPO
	{ 'M'																	, .T. }, ; //X3_TIPO
	{ 10																	, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Obs.Frete Fb'														, .T. }, ; //X3_TITULO
	{ 'Obs.Frete Fb'														, .T. }, ; //X3_TITSPA
	{ 'Obs.Frete Fb'														, .T. }, ; //X3_TITENG
	{ 'Observacao Frete Fob'												, .T. }, ; //X3_DESCRIC
	{ 'Observacao Frete Fob'												, .T. }, ; //X3_DESCSPA
	{ 'Observacao Frete Fob'												, .T. }, ; //X3_DESCENG
	{ ''																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
//
aHlpPor := {}
aAdd( aHlpPor, 'Observação para Frete Fob' )

PutSX1Help( "PA1_I_OBSFO", aHlpPor, {}, {}, .T.,,.T. )
AutoGrLog( "Atualizado o Help do campo " + "A1_I_OBSFO" )
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP39908C" ) ) ) ;
