...
/*/
//--------------------------------------------------------------------
User Function UPD43081( cEmpAmb, cFilAmb )
Local   aSay      := {}
Local   aButton   := {}
Local   aMarcadas := {}
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização realizada.", "UPD43081" )
				Else
					MsgStop( "Atualização não realizada.", "UPD43081" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'ZZL'																	, .T. }, ; //X3_ARQUIVO
	{ '9C'																	, .T. }, ; //X3_ORDEM
	{ 'ZZL_GESCPV'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Gest.Cart.PV'														, .T. }, ; //X3_TITULO
	{ 'Gest.Cart.PV'														, .T. }, ; //X3_TITSPA
	{ 'Gest.Cart.PV'														, .T. }, ; //X3_TITENG
	{ 'Gestor Carteira PV'													, .T. }, ; //X3_DESCRIC
	{ 'Gestor Carteira PV'													, .T. }, ; //X3_DESCSPA
	{ 'Gestor Carteira PV'													, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPD43081" ) ) ) ;
