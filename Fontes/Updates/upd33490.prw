...
/*/
//--------------------------------------------------------------------
User Function UPD33490( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UPD33490" )
				Else
					MsgStop( "Atualização não Realizada.", "UPD33490" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	...
	{ 'R'																	, .T. }, ; //X3_CONTEXT
	{ ''																	, .T. }, ; //X3_OBRIGAT
	{ "IF(Empty(cCInve),.T.,ExistCpo('ZZI',M->C7_I_SUBIN+'2'))"				, .T. }, ; //X3_VLDUSER
	{ ''																	, .T. }, ; //X3_CBOX
	{ ''																	, .T. }, ; //X3_CBOXSPA
	{ ''																	, .T. }, ; //X3_CBOXENG
	{ ''																	, .T. }, ; //X3_PICTVAR
...
aAdd( aSX3, { ;
	...
	Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) + ;
	Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(160)					, .T. }, ; //X3_USADO
	{ 'IF(!INCLUI,Posicione("ZZI",1,xFilial("ZZI") + SC7->C7_I_SUBIN, "ZZI_DESINV"),"")', .T. }, ; //X3_RELACAO
	{ ''																	, .T. }, ; //X3_F3
	{ 0																		, .T. }, ; //X3_NIVEL
	{ Chr(254) + Chr(192)													, .T. }, ; //X3_RESERV
	{ ''																	, .T. }, ; //X3_CHECK
...
aAdd( aSX3, { ;
	...
	{ '1=Investimento Pai;2=Sub-Investimento'								, .T. }, ; //X3_CBOX
...
aAdd( aSIX, { ;
	'ZZI'																	, ; //INDICE
	'1'																		, ; //ORDEM
	'ZZI_FILIAL+ZZI_CODINV+ZZI_TIPO'										, ; //CHAVE
	'Cod.Investim+Tipo'														, ; //DESCRICAO
	''																		, ; //DESCSPA
	''																		, ; //DESCENG
	'U'																		, ; //PROPRI
	''																		, ; //F3
...
aAdd( aSX7, { ;
	'ZZI_INVPAI'															, ; //X7_CAMPO
	'001'																	, ; //X7_SEQUENC
	'POSICIONE("ZZI",1,xFilial("ZZI")+M->ZZI_INVPAI,"ZZI_DTINIC")'			, ; //X7_REGRA
	'ZZI_DTINIC'															, ; //X7_CDOMIN
	'P'																		, ; //X7_TIPO
	'N'																		, ; //X7_SEEK
	''																		, ; //X7_ALIAS
	0																		, ; //X7_ORDEM
...
aAdd( aSX7, { ;
	'ZZI_INVPAI'															, ; //X7_CAMPO
	'002'																	, ; //X7_SEQUENC
	'POSICIONE("ZZI",1,xFilial("ZZI")+M->ZZI_INVPAI,"ZZI_DTFIM")'			, ; //X7_REGRA
	'ZZI_DTFIM'																, ; //X7_CDOMIN
	'P'																		, ; //X7_TIPO
	'N'																		, ; //X7_SEEK
	''																		, ; //X7_ALIAS
...
For nI := 1 To Len( aSX7 )
	...
	If !SX7->( dbSeek( PadR( aSX7[nI][1], nTamSeek ) + aSX7[nI][2] ) )
		...
		If !( aSX7[nI][1] $ cAlias+ "/" + aSX7[nI][2] )
			cAlias += aSX7[nI][1] + "/" + aSX7[nI][2]
...
aAdd( aSXB, { ;
	...
	''																		, ; //XB_DESCENG
	''																		, ; //XB_WCONTEM
	"ZZI->ZZI_MSBLQL <> '1' .AND. ZZI->ZZI_TIPO = '2' .AND. cCInve == ZZI->ZZI_INVPAI"} ) //XB_CONTEM
...
For nI := 1 To Len( aSXB )
	...
	If !Empty( aSXB[nI][1] )
		...
		If !SXB->( dbSeek( PadR( aSXB[nI][1], Len( SXB->XB_ALIAS ) ) + aSXB[nI][2] + aSXB[nI][3] + aSXB[nI][4] ) )
			...
			If !( aSXB[nI][1] $ cAlias+ "/" + aSXB[nI][2] )
				cAlias += aSXB[nI][1] + "/" + aSXB[nI][2]
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPD33490" ) ) ) ;
