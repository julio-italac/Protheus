...
/*/
//--------------------------------------------------------------------
User Function UPD28685( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UPD28685" )
				Else
					MsgStop( "Atualização não Realizada.", "UPD28685" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'ZZL'																	, .T. }, ; //X3_ARQUIVO
	{ 'A3'																	, .T. }, ; //X3_ORDEM
	{ 'ZZL_PEFRPA'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Per.Frac.PA?'														, .T. }, ; //X3_TITULO
	{ 'Per.Frac.PA?'														, .T. }, ; //X3_TITSPA
	{ 'Per.Frac.PA?'														, .T. }, ; //X3_TITENG
	{ 'Permite Fracionar PA?'												, .T. }, ; //X3_DESCRIC
	{ 'Permite Fracionar PA?'												, .T. }, ; //X3_DESCSPA
	{ 'Permite Fracionar PA?'												, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPD28685" ) ) ) ;
