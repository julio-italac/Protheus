...
/*/
//--------------------------------------------------------------------
User Function UP52838( cEmpAmb, cFilAmb )
Local   aSay      := {}
Local   aButton   := {}
Local   aMarcadas := {}
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgInfo( "Atualização realizada.", "UP52838" )
				Else
					MsgStop( "Atualização não realizada.", "UP52838" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'ZGJ'																	, .T. }, ; //X3_ARQUIVO
	{ '12'																	, .T. }, ; //X3_ORDEM
	{ 'ZGJ_ORIGEM'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 60																	, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Orig.Digital'														, .T. }, ; //X3_TITULO
	{ 'Orig.Digital'														, .T. }, ; //X3_TITSPA
	{ 'Orig.Digital'														, .T. }, ; //X3_TITENG
	{ 'Origem da Digitalizacao'												, .T. }, ; //X3_DESCRIC
	{ 'Origem da Digitalizacao'												, .T. }, ; //X3_DESCSPA
	{ 'Origem da Digitalizacao'												, .T. }, ; //X3_DESCENG
	{ ''																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
aAdd( aSX3, { ;
	{ 'ZGJ'																	, .T. }, ; //X3_ARQUIVO
	{ '13'																	, .T. }, ; //X3_ORDEM
	{ 'ZGJ_FISICO'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Ha Canhoto F'														, .T. }, ; //X3_TITULO
	{ 'Ha Canhoto F'														, .T. }, ; //X3_TITSPA
	{ 'Ha Canhoto F'														, .T. }, ; //X3_TITENG
	{ 'Possui Canhoto Fisico'												, .T. }, ; //X3_DESCRIC
	{ 'Possui Canhoto Fisico'												, .T. }, ; //X3_DESCSPA
	{ 'Possui Canhoto Fisico'												, .T. }, ; //X3_DESCENG
	{ ''																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	{ 'x       x       x       x       x       x       x       x       x       x       x       x       x       x       x x', .T. }, ; //X3_USADO
	{ ''																	, .T. }, ; //X3_RELACAO
...
//
aHlpPor := {}
aAdd( aHlpPor, 'Origem da Digitalização' )

aHlpEng := {}
aAdd( aHlpEng, 'Origem da Digitalização' )

aHlpSpa := {}
aAdd( aHlpSpa, 'Origem da Digitalização' )

PutSX1Help( "PZGJ_ORIGEM", aHlpPor, aHlpEng, aHlpSpa, .T.,,.T. )
AutoGrLog( "Atualizado o Help do campo " + "ZGJ_ORIGEM" )

aHlpPor := {}
aAdd( aHlpPor, 'Possui Canhoto Físico' )

aHlpEng := {}
aAdd( aHlpEng, 'Possui Canhoto Físico' )

aHlpSpa := {}
aAdd( aHlpSpa, 'Possui Canhoto Físico' )

PutSX1Help( "PZGJ_FISICO", aHlpPor, aHlpEng, aHlpSpa, .T.,,.T. )
AutoGrLog( "Atualizado o Help do campo " + "ZGJ_FISICO" )
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP52838" ) ) ) ;
