...
/*/
//--------------------------------------------------------------------
User Function UP29638( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UP29638" )
				Else
					MsgStop( "Atualização não Realizada.", "UP29638" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	...
	{ 'Condicao de Pagamento'												, .F. }, ; //X3_DESCRIC
	{ 'Condicion de Pago'													, .F. }, ; //X3_DESCSPA
	{ 'Payment Term'														, .F. }, ; //X3_DESCENG
	{ '@!'																	, .F. }, ; //X3_PICTURE
	{ 'U_AOMS074V()'														, .T. }, ; //X3_VALID
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP29638" ) ) ) ;
