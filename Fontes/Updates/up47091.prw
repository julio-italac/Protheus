...
/*/
//--------------------------------------------------------------------
User Function UP47091( cEmpAmb, cFilAmb )
Local   aSay      := {}
Local   aButton   := {}
Local   aMarcadas := {}
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgInfo( "Atualização realizada.", "UP47091" )
				Else
					MsgStop( "Atualização não realizada.", "UP47091" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'SRA'																	, .T. }, ; //X3_ARQUIVO
	{ 'FW'																	, .T. }, ; //X3_ORDEM
	{ 'RA_I_CARVT'															, .T. }, ; //X3_CAMPO
	{ 'N'																	, .T. }, ; //X3_TIPO
	{ 10																	, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'N Cartao VT'															, .T. }, ; //X3_TITULO
	{ 'N Cartao VT'															, .T. }, ; //X3_TITSPA
	{ 'N Cartao VT'															, .T. }, ; //X3_TITENG
	{ 'Numero do Cartao de VT'												, .T. }, ; //X3_DESCRIC
	{ 'Numero do Cartao de VT'												, .T. }, ; //X3_DESCSPA
	{ 'Numero do Cartao de VT'												, .T. }, ; //X3_DESCENG
	{ '9999999999'															, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	{ 'x      xx       x       x       x       x       x       x       x       x       x       x       x       x       x', .T. }, ; //X3_USADO
	{ ''																	, .T. }, ; //X3_RELACAO
	{ ''																	, .T. }, ; //X3_F3
	{ 0																		, .T. }, ; //X3_NIVEL
...
_lRet:=.T.
Fwmsgrun(,{|oproc| _lRet:=MyOpenSm0(.F.)},"Aguarde...","Tentando abrir SIGAMAT...")
If !_lRet
//If !MyOpenSm0(.F.)
	Return aRet
EndIf
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP47091" ) ) ) ;
