...
/*/
//--------------------------------------------------------------------
User Function UP38531D( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UP38531D" )
				Else
					MsgStop( "Atualização não Realizada.", "UP38531D" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'SA2'																	, .T. }, ; //X3_ARQUIVO
	{ 'FR'																	, .T. }, ; //X3_ORDEM
	{ 'A2_L_TPASS'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Tipo Associa'														, .T. }, ; //X3_TITULO
	{ 'Tipo Associa'														, .T. }, ; //X3_TITSPA
	{ 'Tipo Associa'														, .T. }, ; //X3_TITENG
	{ 'Tipo de Associacao/Cooper'											, .T. }, ; //X3_DESCRIC
	{ 'Tipo de Associacao/Cooper'											, .T. }, ; //X3_DESCSPA
	{ 'Tipo de Associacao/Cooper'											, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	...
	{ 'R'																	, .T. }, ; //X3_CONTEXT
	{ ''																	, .T. }, ; //X3_OBRIGAT
	{ 'Vazio() .Or. Pertence("ACN")'										, .T. }, ; //X3_VLDUSER
	{ 'A=Associacao-Cooperativa;C=Associado-Cooperado;N=Produtor-Normal'	, .T. }, ; //X3_CBOX
	{ ''																	, .T. }, ; //X3_CBOXSPA
	{ ''																	, .T. }, ; //X3_CBOXENG
	{ ''																	, .T. }, ; //X3_PICTVAR
	{ ''																	, .T. }, ; //X3_WHEN
...
PutSX1Help( "PA2_L_TPASS", aHlpPor, {}, {}, .T.,,.T. )
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP38531D" ) ) ) ;
