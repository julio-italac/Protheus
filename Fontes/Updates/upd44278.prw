...
/*/
//--------------------------------------------------------------------
User Function UPD44278( cEmpAmb, cFilAmb )
Local   aSay      := {}
Local   aButton   := {}
Local   aMarcadas := {}
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização realizada.", "UPD44278" )
				Else
					MsgStop( "Atualização não realizada.", "UPD44278" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'SA3'																	, .T. }, ; //X3_ARQUIVO
	{ 'W7'																	, .T. }, ; //X3_ORDEM
	{ 'A3_I_ESTOQ'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Val.Estoq?'															, .T. }, ; //X3_TITULO
	{ 'Val.Estoq?'															, .T. }, ; //X3_TITSPA
	{ 'Val.Estoq?'															, .T. }, ; //X3_TITENG
	{ 'Valida Estq Broker?'													, .T. }, ; //X3_DESCRIC
	{ 'Valida Estq Broker?'													, .T. }, ; //X3_DESCSPA
	{ 'Valida Estq Broker?'													, .T. }, ; //X3_DESCENG
	{ '!@'																	, .T. }, ; //X3_PICTURE
...
aAdd( aSIX, { ;
	'SZW'																	, ; //INDICE
	'H'																		, ; //ORDEM
	'ZW_FILIAL+ZW_PEDIMPO+ZW_VEND1'											, ; //CHAVE
	'Pedido Importado'														, ; //DESCRICAO
	'Pedido Importado'														, ; //DESCSPA
	'Pedido Importado'														, ; //DESCENG
	'U'																		, ; //PROPRI
	''																		, ; //F3
	'PEIMPVEND1'															, ; //NICKNAME
	'S'																		} ) //SHOWPESQ
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPD44278" ) ) ) ;
