...
/*/
//--------------------------------------------------------------------
User Function UP40740( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UP40740" )
				Else
					MsgStop( "Atualização não Realizada.", "UP40740" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'ZZL'																	, .T. }, ; //X3_ARQUIVO
	{ '9T'																	, .T. }, ; //X3_ORDEM
	{ 'ZZL_ALTLJT'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Alt.Loj.FatT'														, .T. }, ; //X3_TITULO
	{ 'Alt.Loj.FatT'														, .T. }, ; //X3_TITSPA
	{ 'Alt.Loj.FatT'														, .T. }, ; //X3_TITENG
	{ 'Altera Loja Fatura Transp'											, .T. }, ; //X3_DESCRIC
	{ 'Altera Loja Fatura Transp'											, .T. }, ; //X3_DESCSPA
	{ 'Altera Loja Fatura Transp'											, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
//
aHlpPor := {}
aAdd( aHlpPor, 'Altera Loja na geração de Fatura para' )
aAdd( aHlpPor, 'Transportadora?' )

PutSX1Help( "PZZL_ALTLJT", aHlpPor, {}, {}, .T.,,.T. )
AutoGrLog( "Atualizado o Help do campo " + "ZZL_ALTLJT" )
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP40740" ) ) ) ;
