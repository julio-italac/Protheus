...
/*/
//--------------------------------------------------------------------
User Function UP49586( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UP49586" )
				Else
					MsgStop( "Atualização não Realizada.", "UP49586" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	...
	{ 'CNPJ do Embarcador'													, .T. }, ; //X3_DESCRIC
	{ 'CNPJ do Embarcador'													, .T. }, ; //X3_DESCSPA
	{ 'CNPJ do Embarcador'													, .T. }, ; //X3_DESCENG
	...
	{ 'R'																	, .T. }, ; //X3_CONTEXT
	{ ''																	, .T. }, ; //X3_OBRIGAT
	{ 'Vazio() .Or. ExistCPO( "SA2", M->ZEL_CNPJEM, 3 )'					, .T. }, ; //X3_VLDUSER
	{ ''																	, .T. }, ; //X3_CBOX
	{ ''																	, .T. }, ; //X3_CBOXSPA
	{ ''																	, .T. }, ; //X3_CBOXENG
	{ ''																	, .T. }, ; //X3_PICTVAR
...
aAdd( aSIX, { ;
	'ZEL'																	, ; //INDICE
	'4'																		, ; //ORDEM
	'ZEL_FILIAL+ZEL_FILTMS'													, ; //CHAVE
	'Filial TMS'															, ; //DESCRICAO
	'Filial TMS'															, ; //DESCSPA
	'Filial TMS'															, ; //DESCENG
	'U'																		, ; //PROPRI
	''																		, ; //F3
...
aAdd( aSIX, { ;
	'ZEL'																	, ; //INDICE
	'5'																		, ; //ORDEM
	'ZEL_FILIAL+ZEL_CNPJEM'													, ; //CHAVE
	'CNPJ Embarcador'														, ; //DESCRICAO
	'CNPJ Embarcador'														, ; //DESCSPA
	'CNPJ Embarcador'														, ; //DESCENG
	'U'																		, ; //PROPRI
	''																		, ; //F3
	''																		, ; //NICKNAME
	'S'																		} ) //SHOWPESQ
...
PutSX1Help( "PZEL_CNPJEM", aHlpPor, {}, {}, .T.,,.T. )
AutoGrLog( "Atualizado o Help do campo " + "ZEL_CNPJEM" )
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP49586" ) ) ) ;
