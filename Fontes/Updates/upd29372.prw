...
/*/
//--------------------------------------------------------------------
User Function UPD29372( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UPD29372" )
				Else
					MsgStop( "Atualização não Realizada.", "UPD29372" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'SA2'																	, .T. }, ; //X3_ARQUIVO
	{ 'U6'																	, .T. }, ; //X3_ORDEM
	{ 'A2_I_TPFRT'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Tp Seg Frete'														, .T. }, ; //X3_TITULO
	{ 'Tp Seg Frete'														, .T. }, ; //X3_TITSPA
	{ 'Tp Seg Frete'														, .T. }, ; //X3_TITENG
	{ 'Tipo do Seguro do Frete'												, .T. }, ; //X3_DESCRIC
	{ 'Tipo do Seguro do Frete'												, .T. }, ; //X3_DESCSPA
	{ 'Tipo do Seguro do Frete'												, .T. }, ; //X3_DESCENG
	{ ''																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	...
	{ ''																	, .T. }, ; //X3_OBRIGAT
	{ ''																	, .T. }, ; //X3_VLDUSER
	{ 'S=Desconta Seguro;N=Nao Desconta Seguro;C=Seguro no CTE'				, .T. }, ; //X3_CBOX
	{ ''																	, .T. }, ; //X3_CBOXSPA
	{ ''																	, .T. }, ; //X3_CBOXENG
	{ ''																	, .T. }, ; //X3_PICTVAR
	{ ''																	, .T. }, ; //X3_WHEN
...
aAdd( aSX3, { ;
	{ 'SF2'																	, .T. }, ; //X3_ARQUIVO
	{ 'R3'																	, .T. }, ; //X3_ORDEM
	{ 'F2_I_NDFSE'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 27																	, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'NDF Seguro'															, .T. }, ; //X3_TITULO
	{ 'NDF Seguro'															, .T. }, ; //X3_TITSPA
	{ 'NDF Seguro'															, .T. }, ; //X3_TITENG
	{ 'NDF Seguro'															, .T. }, ; //X3_DESCRIC
	{ 'NDF Seguro'															, .T. }, ; //X3_DESCSPA
	{ 'NDF Seguro'															, .T. }, ; //X3_DESCENG
	{ ''																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	{ Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) + ;
	Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) + ;
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPD29372" ) ) ) ;
