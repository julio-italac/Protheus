...
/*/
//--------------------------------------------------------------------
User Function UP50278( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UP50278" )
				Else
					MsgStop( "Atualização não Realizada.", "UP50278" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'ZZL'																	, .T. }, ; //X3_ARQUIVO
	{ 'A7'																	, .T. }, ; //X3_ORDEM
	{ 'ZZL_VLMAXP'															, .T. }, ; //X3_CAMPO
	{ 'N'																	, .T. }, ; //X3_TIPO
	{ 14																	, .T. }, ; //X3_TAMANHO
	{ 2																		, .T. }, ; //X3_DECIMAL
	{ 'Max.Ap.Prosp'														, .T. }, ; //X3_TITULO
	{ 'Max.Ap.Prosp'														, .T. }, ; //X3_TITSPA
	{ 'Max.Ap.Prosp'														, .T. }, ; //X3_TITENG
	{ 'Vl.Max Permitido Aprov.Pr'											, .T. }, ; //X3_DESCRIC
	{ 'Vl.Max Permitido Aprov.Pr'											, .T. }, ; //X3_DESCSPA
	{ 'Vl.Max Permitido Aprov.Pr'											, .T. }, ; //X3_DESCENG
...
//
aHlpPor := {}
aAdd( aHlpPor, 'Valor Max Permitido para Aprovação do' )
aAdd( aHlpPor, 'Prospect' )

PutSX1Help( "PZZL_VLMAXP", aHlpPor, {}, {}, .T.,,.T. )
AutoGrLog( "Atualizado o Help do campo " + "ZZL_VLMAXP" )
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP50278" ) ) ) ;
