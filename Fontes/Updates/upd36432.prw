...
/*/
//--------------------------------------------------------------------
User Function UPD36432( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UPD36432" )
				Else
					MsgStop( "Atualização não Realizada.", "UPD36432" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	...
	{ 'Valor do frete por quilo'											, .T. }, ; //X3_DESCRIC
	{ 'Valor do frete por quilo'											, .T. }, ; //X3_DESCSPA
	{ 'Valor do frete por quilo'											, .T. }, ; //X3_DESCENG
...
aAdd( aSX3, { ;
	...
	{ 'Veiculo'																, .T. }, ; //X3_TITSPA
	{ 'Veiculo'																, .T. }, ; //X3_TITENG
	{ 'Veiculo'																, .T. }, ; //X3_DESCRIC
	{ 'Veiculo'																, .T. }, ; //X3_DESCSPA
	{ 'Veiculo'																, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ 'IF(Empty(M->A2_I_CAMIN),.T.,ExistCpo("DA3",M->A2_I_CAMIN))'			, .T. }, ; //X3_VALID
	{ 'x       x       x       x       x       x       x       x       x       x       x       x       x       x       x x', .T. }, ; //X3_USADO
	{ ''																	, .T. }, ; //X3_RELACAO
	{ 'DA3'																	, .T. }, ; //X3_F3
	{ 0																		, .T. }, ; //X3_NIVEL
...
aAdd( aSX3, { ;
	{ 'SA2'																	, .T. }, ; //X3_ARQUIVO
	{ 'ZZ'																	, .T. }, ; //X3_ORDEM
	{ 'A2_I_MOTOR'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 6																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Motorista'															, .T. }, ; //X3_TITULO
	{ 'Motorista'															, .T. }, ; //X3_TITSPA
	{ 'Motorista'															, .T. }, ; //X3_TITENG
	{ 'Motorista'															, .T. }, ; //X3_DESCRIC
	{ 'Motorista'															, .T. }, ; //X3_DESCSPA
	{ 'Motorista'															, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ 'IF(Empty(M->A2_I_MOTOR),.T.,ExistCpo("DA4",M->A2_I_MOTOR))'			, .T. }, ; //X3_VALID
	{ 'x       x       x       x       x       x       x       x       x       x       x       x       x       x       x x', .T. }, ; //X3_USADO
	{ ''																	, .T. }, ; //X3_RELACAO
	{ 'DA4'																	, .T. }, ; //X3_F3
	{ 0																		, .T. }, ; //X3_NIVEL
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPD36432" ) ) ) ;
