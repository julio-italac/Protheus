...
/*/
//--------------------------------------------------------------------
User Function UPD38855( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UPD38855" )
				Else
					MsgStop( "Atualização não Realizada.", "UPD38855" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'ZZL'																	, .T. }, ; //X3_ARQUIVO
	{ '42'																	, .T. }, ; //X3_ORDEM
	{ 'ZZL_FLAPSA'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 250																	, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Fil.Aprov.SA'														, .T. }, ; //X3_TITULO
	{ 'Fil.Aprov.SA'														, .T. }, ; //X3_TITSPA
	{ 'Fil.Aprov.SA'														, .T. }, ; //X3_TITENG
	{ 'Filiais de aprovacao da S'											, .T. }, ; //X3_DESCRIC
	{ 'Filiais de aprovacao da S'											, .T. }, ; //X3_DESCSPA
	{ 'Filiais de aprovacao da S'											, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPD38855" ) ) ) ;
