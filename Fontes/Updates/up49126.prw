...
/*/
//--------------------------------------------------------------------
User Function UP49126( cEmpAmb, cFilAmb )
Local   aSay      := {}
Local   aButton   := {}
Local   aMarcadas := {}
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgInfo( "Atualização realizada.", "UP49126" )
				Else
					MsgStop( "Atualização não realizada.", "UP49126" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'ZF5'																	, .T. }, ; //X3_ARQUIVO
	{ '11'																	, .T. }, ; //X3_ORDEM
	{ 'ZF5_ESTONO'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Estornada?'															, .T. }, ; //X3_TITULO
	{ 'Estornada?'															, .T. }, ; //X3_TITSPA
	{ 'Estornada?'															, .T. }, ; //X3_TITENG
	{ 'Ocorrencia Estornada?'												, .T. }, ; //X3_DESCRIC
	{ 'Ocorrencia Estornada?'												, .T. }, ; //X3_DESCSPA
	{ 'Ocorrencia Estornada?'												, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
aAdd( aSX3, { ;
	{ 'ZF5'																	, .T. }, ; //X3_ARQUIVO
	{ '12'																	, .T. }, ; //X3_ORDEM
	{ 'ZF5_DTATUA'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Atu.Dt.NF?'															, .T. }, ; //X3_TITULO
	{ 'Atu.Dt.NF?'															, .T. }, ; //X3_TITSPA
	{ 'Atu.Dt.NF?'															, .T. }, ; //X3_TITENG
	{ 'Atualizou Data da Nota?'												, .T. }, ; //X3_DESCRIC
	{ 'Atualizou Data da Nota?'												, .T. }, ; //X3_DESCSPA
	{ 'Atualizou Data da Nota?'												, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	{ 'x       x       x       x       x       x       x       x       x       x       x       x       x       x       x x', .T. }, ; //X3_USADO
	{ ''																	, .T. }, ; //X3_RELACAO
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP49126" ) ) ) ;
