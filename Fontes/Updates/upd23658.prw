...
/*/
//--------------------------------------------------------------------
User Function UPD23658( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UPD23658" )
				Else
					MsgStop( "Atualização não Realizada.", "UPD23658" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	...
	{ 'Nombre'																, .T. }, ; //X3_TITSPA
	{ 'Name'																, .T. }, ; //X3_TITENG
	{ 'Nome do Funcionário.'												, .T. }, ; //X3_DESCRIC
	{ 'Nombre del empleado.'												, .T. }, ; //X3_DESCSPA
	{ 'Employee Name'														, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPD23658" ) ) ) ;
