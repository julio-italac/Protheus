...
/*/
//--------------------------------------------------------------------
User Function UPD42562( cEmpAmb, cFilAmb )
Local   aSay      := {}
Local   aButton   := {}
Local   aMarcadas := {}
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização realizada.", "UPD42562" )
				Else
					MsgStop( "Atualização não realizada.", "UPD42562" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'SA2'																	, .T. }, ; //X3_ARQUIVO
	{ 'FC'																	, .T. }, ; //X3_ORDEM
	{ 'A2_I_LJTRN'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Lj.Ativa TRN'														, .T. }, ; //X3_TITULO
	{ 'Lj.Ativa TRN'														, .T. }, ; //X3_TITSPA
	{ 'Lj.Ativa TRN'														, .T. }, ; //X3_TITENG
	{ 'Loja Ativa Troca Nota'												, .T. }, ; //X3_DESCRIC
	{ 'Loja Ativa Troca Nota'												, .T. }, ; //X3_DESCSPA
	{ 'Loja Ativa Troca Nota'												, .T. }, ; //X3_DESCENG
	{ ''																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPD42562" ) ) ) ;
