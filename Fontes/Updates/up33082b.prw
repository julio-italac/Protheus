...
/*/
//--------------------------------------------------------------------
User Function UP33082B( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UP33082B" )
				Else
					MsgStop( "Atualização não Realizada.", "UP33082B" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'SC5'																	, .T. }, ; //X3_ARQUIVO
	{ 'XX'																	, .T. }, ; //X3_ORDEM
	{ 'C5_I_PEDOP'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 8																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'PV Oper.Log.'														, .T. }, ; //X3_TITULO
	{ 'PV Oper.Log.'														, .T. }, ; //X3_TITSPA
	{ 'PV Oper.Log.'														, .T. }, ; //X3_TITENG
	{ 'Pedido do Operador Logist'											, .T. }, ; //X3_DESCRIC
	{ 'Pedido do Operador Logist'											, .T. }, ; //X3_DESCSPA
	{ 'Pedido do Operador Logist'											, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP33082B" ) ) ) ;
