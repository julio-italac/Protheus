...
/*/
//--------------------------------------------------------------------
User Function UP49733( cEmpAmb, cFilAmb )
Local   aSay      := {}
Local   aButton   := {}
Local   aMarcadas := {}
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgInfo( "Atualização realizada.", "UP49733" )
				Else
					MsgStop( "Atualização não realizada.", "UP49733" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'ZF5'																	, .T. }, ; //X3_ARQUIVO
	{ '41'																	, .T. }, ; //X3_ORDEM
	{ 'ZF5_CAUCUS'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 2																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Causador?'															, .T. }, ; //X3_TITULO
	{ 'Causador?'															, .T. }, ; //X3_TITSPA
	{ 'Causador?'															, .T. }, ; //X3_TITENG
	{ 'Causador do Custo'													, .T. }, ; //X3_DESCRIC
	{ 'Causador do Custo'													, .T. }, ; //X3_DESCSPA
	{ 'Causador do Custo'													, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
aAdd( aSX5, { ;
	...
	'00'																	, ; //X5_TABELA
	'Z2'																	, ; //X5_CHAVE
	'Causador do Custo'														, ; //X5_DESCRI
	'Causador do Custo'														, ; //X5_DESCSPA
	'Causador do Custo'														} ) //X5_DESCENG
...
aAdd( aSX5, { ;
	...
	'01'																	, ; //X5_CHAVE
	'Logística'																, ; //X5_DESCRI
	'Logística'																, ; //X5_DESCSPA
	'Logística'																} ) //X5_DESCENG
...
aAdd( aSX5, { ;
	...
	'03'																	, ; //X5_CHAVE
	'Expedição'																, ; //X5_DESCRI
	'Expedição'																, ; //X5_DESCSPA
	'Expedição'																} ) //X5_DESCENG
...
aAdd( aSX5, { ;
	...
	'04'																	, ; //X5_CHAVE
	'Qualidade'																, ; //X5_DESCRI
	'Qualidade'																, ; //X5_DESCSPA
	'Qualidade'																} ) //X5_DESCENG

aAdd( aSX5, { ;
	cFilSX5																	, ; //X5_FILIAL
	'Z2'																	, ; //X5_TABELA
	'05'																	, ; //X5_CHAVE
	'Financeiro'															, ; //X5_DESCRI
	'Financeiro'															, ; //X5_DESCSPA
	'Financeiro'															} ) //X5_DESCENG
...
aAdd( aSX5, { ;
	...
	'06'																	, ; //X5_CHAVE
	'Comercial'																, ; //X5_DESCRI
	'Comercial'																, ; //X5_DESCSPA
	'Comercial'																} ) //X5_DESCENG
...
aAdd( aSX5, { ;
	...
	'07'																	, ; //X5_CHAVE
	'Diretoria'																, ; //X5_DESCRI
	'Diretoria'																, ; //X5_DESCSPA
	'Diretoria'																} ) //X5_DESCENG
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP49733" ) ) ) ;
