...
/*/
//--------------------------------------------------------------------
User Function UP21157( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UP21157" )
				Else
					MsgStop( "Atualização não Realizada.", "UP21157" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	...
	{ 'R'																	, .T. }, ; //X3_CONTEXT
	{ ''																	, .T. }, ; //X3_OBRIGAT
	{ 'Vazio().or.If(!Altera,ExistCpo("SA1",U_AFIN004Q("ZAE_CLI")+U_AFIN004Q("ZAE_LOJA")),ExistCpo("SA1",TRBZAE->ZAE_CLI+M->ZAE_LOJA))', .T. }, ; //X3_VLDUSER
	{ ''																	, .T. }, ; //X3_CBOX
	{ ''																	, .T. }, ; //X3_CBOXSPA
	{ ''																	, .T. }, ; //X3_CBOXENG
	{ ''																	, .T. }, ; //X3_PICTVAR
...
aAdd( aSXB, { ;
	...
	'01'																	, ; //XB_SEQ
	'RE'																	, ; //XB_COLUNA
	'Múltiplos Grupos'														, ; //XB_DESCRI
	'Múltiplos Grupos'														, ; //XB_DESCSPA
	'Múltiplos Grupos'														, ; //XB_DESCENG
	''																		, ; //XB_WCONTEM
	'SBM'																	} ) //XB_CONTEM
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP21157" ) ) ) ;
