...
/*/
//--------------------------------------------------------------------
User Function UPD50140( cEmpAmb, cFilAmb )
Local   aSay      := {}
Local   aButton   := {}
Local   aMarcadas := {}
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgInfo( "Atualização realizada.", "UPD50140" )
				Else
					MsgStop( "Atualização não realizada.", "UPD50140" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSIX, { ;
	'ZLJ'																	, ; //INDICE
	'7'																		, ; //ORDEM
	'ZLJ_FILIAL+ZLJ_VFILHA+ZLJ_VIAGEM'										, ; //CHAVE
	'Viagem Filha + Viagem'													, ; //DESCRICAO
	'Viagem Filha + Viagem'													, ; //DESCSPA
	'Viagem Filha + Viagem'													, ; //DESCENG
	'U'																		, ; //PROPRI
	''																		, ; //F3
	'FILHADAMAE'															, ; //NICKNAME
	'S'																		} ) //SHOWPESQ
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPD50140" ) ) ) ;
