...
/*/
//--------------------------------------------------------------------
User Function UPD45768( cEmpAmb, cFilAmb ) 
Local   aSay      := {}
Local   aButton   := {}
Local   aMarcadas := {}
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização realizada.", "UPD45768" )
				Else
					MsgStop( "Atualização não realizada.", "UPD45768" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'ZFC'																	, .T. }, ; //X3_ARQUIVO
	{ '11'																	, .T. }, ; //X3_ORDEM
	{ 'ZFC_ENVREP'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Envia Email?'														, .T. }, ; //X3_TITULO
	{ 'Envia Email?'														, .T. }, ; //X3_TITSPA
	{ 'Envia Email?'														, .T. }, ; //X3_TITENG
	{ 'Envia Email?'														, .T. }, ; //X3_DESCRIC
	{ 'Envia Email?'														, .T. }, ; //X3_DESCSPA
	{ 'Envia Email?'														, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPD45768" ) ) ) ;
