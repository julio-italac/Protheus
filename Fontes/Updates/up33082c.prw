...
/*/
//--------------------------------------------------------------------
User Function UP33082C( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UP33082C" )
				Else
					MsgStop( "Atualização não Realizada.", "UP33082C" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'CC2'																	, .T. }, ; //X3_ARQUIVO
	{ '19'																	, .T. }, ; //X3_ORDEM
	{ 'CC2_I_ENVO'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Env.Aut.OPL?'														, .T. }, ; //X3_TITULO
	{ 'Env.Aut.OPL?'														, .T. }, ; //X3_TITSPA
	{ 'Env.Aut.OPL?'														, .T. }, ; //X3_TITENG
	{ 'Envia automatico p/ Opera'											, .T. }, ; //X3_DESCRIC
	{ 'Envia automatico p/ Opera'											, .T. }, ; //X3_DESCSPA
	{ 'Envia automatico p/ Opera'											, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP33082C" ) ) ) ;
