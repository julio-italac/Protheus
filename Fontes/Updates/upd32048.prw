...
/*/
//--------------------------------------------------------------------
User Function UPD32048( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UPD32048" )
				Else
					MsgStop( "Atualização não Realizada.", "UPD32048" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX1, { ;
	'REST010'																, ; //X1_GRUPO
	'08'																	, ; //X1_ORDEM
	'Avaliar estoque automat?'												, ; //X1_PERGUNT
	''																		, ; //X1_PERSPA
	''																		, ; //X1_PERENG
	'MV_CH0'																, ; //X1_VARIAVL
	'N'																		, ; //X1_TIPO
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPD32048" ) ) ) ;
