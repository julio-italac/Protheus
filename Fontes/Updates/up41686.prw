...
/*/
//--------------------------------------------------------------------
User Function UP41686( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UP41686" )
				Else
					MsgStop( "Atualização não Realizada.", "UP41686" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	...
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ 'Vazio() .or. ExistCpo("SX5","S0"+M->BZ_ORIGEM)'						, .T. }, ; //X3_VALID
	{ 'x xxxx  xxxxx xxx       x      xx       x   x xxxxxx    x       x       x       x       x       x       x     xxx', .T. }, ; //X3_USADO
	{ ''																	, .T. }, ; //X3_RELACAO
	{ 'S0'																	, .T. }, ; //X3_F3
	{ 1																		, .T. }, ; //X3_NIVEL
	{ '    x  x'															, .T. }, ; //X3_RESERV
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP41686" ) ) ) ;
