...
/*/
//--------------------------------------------------------------------
User Function UP41205( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UP41205" )
				Else
					MsgStop( "Atualização não Realizada.", "UP41205" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'SBZ'																	, .T. }, ; //X3_ARQUIVO
	{ '95'																	, .T. }, ; //X3_ORDEM
	{ 'BZ_I_PR3UM'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Controle 3UM'														, .T. }, ; //X3_TITULO
	{ 'Controle 3UM'														, .T. }, ; //X3_TITSPA
	{ 'Controle 3UM'														, .T. }, ; //X3_TITENG
	{ 'Controle Sobre 3 UM?'												, .T. }, ; //X3_DESCRIC
	{ 'Controle Sobre 3 UM?'												, .T. }, ; //X3_DESCSPA
	{ 'Controle Sobre 3 UM?'												, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
//
aHlpPor := {}
aAdd( aHlpPor, 'Controle sobre 3 unidade de medida do' )
aAdd( aHlpPor, 'produto?' )

PutSX1Help( "PBZ_I_PR3UM", aHlpPor, {}, {}, .T.,,.T. )
AutoGrLog( "Atualizado o Help do campo " + "BZ_I_PR3UM" )
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP41205" ) ) ) ;
