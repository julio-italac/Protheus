...
/*/
//--------------------------------------------------------------------
User Function UP47958( cEmpAmb, cFilAmb )
Local   aSay      := {}
Local   aButton   := {}
Local   aMarcadas := {}
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgInfo( "Atualização realizada.", "UP47958" )
				Else
					MsgStop( "Atualização não realizada.", "UP47958" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	...
	{ ''																	, .T. }, ; //X3_OBRIGAT
	{ ''																	, .T. }, ; //X3_VLDUSER
	{ 'V=VRB;D=DEV;M=MUL;O=OUT;P=DIF'										, .T. }, ; //X3_CBOX
	{ ''																	, .T. }, ; //X3_CBOXSPA
	{ ''																	, .T. }, ; //X3_CBOXENG
	{ ''																	, .T. }, ; //X3_PICTVAR
	{ ''																	, .T. }, ; //X3_WHEN
...
aAdd( aSX3, { ;
	...
	{ 30																	, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Gestor/Resp'															, .T. }, ; //X3_TITULO
	{ 'Gestor/Resp'															, .T. }, ; //X3_TITSPA
	{ 'Gestor/Resp'															, .T. }, ; //X3_TITENG
	{ 'Gestor/Responsavel'													, .T. }, ; //X3_DESCRIC
	{ 'Gestor/Responsavel'													, .T. }, ; //X3_DESCSPA
	{ 'Gestor/Responsavel'													, .T. }, ; //X3_DESCENG
	{ ''																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	{ 'x       x       x       x       x       x       x       x       x       x       x       x       x       x       x x', .T. }, ; //X3_USADO
	{ ''																	, .T. }, ; //X3_RELACAO
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP47958" ) ) ) ;
