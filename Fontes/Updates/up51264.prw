...
/*/
//--------------------------------------------------------------------
User Function UP51264( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UP51264" )
				Else
					MsgStop( "Atualização não Realizada.", "UP51264" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSXB, { ;
	...
	''																		, ; //XB_DESCENG
	''																		, ; //XB_WCONTEM
	"SA3->A3_MSBLQL <> '1' .And. SA3->A3_I_TIPV == 'V'"						} ) //XB_CONTEM
...
//
aHlpPor := {}
aAdd( aHlpPor, 'Código da Vendedor' )

aHlpEng := {}
aAdd( aHlpEng, 'Código da Vendedor' )

aHlpSpa := {}
aAdd( aHlpSpa, 'Código da Vendedor' )
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP51264" ) ) ) ;
