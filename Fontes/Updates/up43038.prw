...
/*/
//--------------------------------------------------------------------
User Function UP43038( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UP43038" )
				Else
					MsgStop( "Atualização não Realizada.", "UP43038" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	...
	{ 'Codigo del usuario'													, .T. }, ; //X3_DESCSPA
	{ 'User code'															, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
aAdd( aSX3, { ;
	...
	{ ''																	, .T. }, ; //X3_VALID
	{ 'x       x       x       x       x       x       x       x       x       x       x       x       x       x       x x', .T. }, ; //X3_USADO
	{ 'POSICIONE("CB1",1,XFILIAL("CB1")+M->CBB_USU,"CB1_NOME")'				, .T. }, ; //X3_RELACAO
	{ ''																	, .T. }, ; //X3_F3
	{ 0																		, .T. }, ; //X3_NIVEL
	...
	{ ''																	, .T. }, ; //X3_PICTVAR
	{ ''																	, .F. }, ; //X3_WHEN
	{ 'POSICIONE("CB1",1,XFILIAL("CB1")+CBB->CBB_USU,"CB1_NOME")'			, .T. }, ; //X3_INIBRW
	{ ''																	, .T. }, ; //X3_GRPSXG
	{ ''																	, .T. }, ; //X3_FOLDER
	{ ''																	, .T. }, ; //X3_CONDSQL
	{ ''																	, .T. }, ; //X3_CHKSQL
...
aAdd( aSX7, { ;
	'CBB_USU'																, ; //X7_CAMPO
	'001'																	, ; //X7_SEQUENC
	'CB1->CB1_NOME")'														, ; //X7_REGRA
	'CBB_NUSU'																, ; //X7_CDOMIN
	'P'																		, ; //X7_TIPO
	'S'																		, ; //X7_SEEK
	'CB1'																	, ; //X7_ALIAS
	1																		, ; //X7_ORDEM
	'XFILIAL("CB1")+M->CBB_USU'												, ; //X7_CHAVE
	'U'																		, ; //X7_PROPRI
	''																		} ) //X7_CONDIC
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP43038" ) ) ) ;
