...
/*/
//--------------------------------------------------------------------
User Function UPD32075( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UPD32075" )
				Else
					MsgStop( "Atualização não Realizada.", "UPD32075" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	...
	{ 'Risco'																, .F. }, ; //X3_TITSPA
	{ 'Risco'																, .F. }, ; //X3_TITENG
	{ 'Grau de Risco'														, .F. }, ; //X3_DESCRIC
	{ 'Grau de Risco'														, .F. }, ; //X3_DESCSPA
	{ 'Grau de Risco'														, .F. }, ; //X3_DESCENG
	{ '@!'																	, .F. }, ; //X3_PICTURE
	{ ''																	, .F. }, ; //X3_VALID
	...
	{ '€'																	, .F. }, ; //X3_OBRIGAT
	{ ''																	, .F. }, ; //X3_VLDUSER
	{ 'A=Risco A;B=Risco B;C=Risco C;D=Risco D; E=Risco E'					, .F. }, ; //X3_CBOX
	{ 'A=Risco A;B=Risco B;C=Risco C;D=Risco D; E=Risco E'					, .F. }, ; //X3_CBOXSPA
	{ 'A=Risco A;B=Risco B;C=Risco C;D=Risco D; E=Risco E'					, .F. }, ; //X3_CBOXENG
	{ ''																	, .F. }, ; //X3_PICTVAR
	{ ''																	, .F. }, ; //X3_WHEN
	{ ''																	, .F. }, ; //X3_INIBRW
	{ ''																	, .F. }, ; //X3_GRPSXG
...
aAdd( aSX3, { ;
	{ 'SA3'																	, .F. }, ; //X3_ARQUIVO
	{ '9K'																	, .F. }, ; //X3_ORDEM
	{ 'A3_I_TABPR'															, .F. }, ; //X3_CAMPO
	{ 'C'																	, .F. }, ; //X3_TIPO
	{ 3																		, .F. }, ; //X3_TAMANHO
	{ 0																		, .F. }, ; //X3_DECIMAL
	{ 'Tab. Preco'															, .F. }, ; //X3_TITULO
	{ 'Tab. Preco'															, .F. }, ; //X3_TITSPA
	{ 'Tab. Preco'															, .F. }, ; //X3_TITENG
	{ 'Tab. Preco'															, .F. }, ; //X3_DESCRIC
	{ 'Tab. Preco'															, .F. }, ; //X3_DESCSPA
	{ 'Tab. Preco'															, .F. }, ; //X3_DESCENG
	{ '@!'																	, .F. }, ; //X3_PICTURE
	{ 'ExistCpo("DA0",M->A3_I_TABPR)'										, .F. }, ; //X3_VALID
	{ Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) + ;
	Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) + ;
	Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(160)					, .F. }, ; //X3_USADO
	{ ''																	, .F. }, ; //X3_RELACAO
...
aAdd( aSX3, { ;
	...
	{ 14																	, .F. }, ; //X3_TAMANHO
	{ 2																		, .F. }, ; //X3_DECIMAL
	{ 'Limite Cred'															, .F. }, ; //X3_TITULO
	{ 'Limite Cred'															, .F. }, ; //X3_TITSPA
	{ 'Limite Cred'															, .F. }, ; //X3_TITENG
	{ 'Limite Credito'														, .F. }, ; //X3_DESCRIC
	{ 'Limite Credito'														, .F. }, ; //X3_DESCSPA
	{ 'Limite Credito'														, .F. }, ; //X3_DESCENG
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPD32075" ) ) ) ;
