...
/*/
//--------------------------------------------------------------------
User Function UP21436( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UP21436" )
				Else
					MsgStop( "Atualização não Realizada.", "UP21436" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'ZFR'																	, .T. }, ; //X3_ARQUIVO
	{ '36'																	, .T. }, ; //X3_ORDEM
	{ 'ZFR_FATOPA'															, .T. }, ; //X3_CAMPO
	{ 'N'																	, .T. }, ; //X3_TIPO
	{ 18																	, .T. }, ; //X3_TAMANHO
	{ 6																		, .T. }, ; //X3_DECIMAL
	{ 'Fator Conv.P'														, .T. }, ; //X3_TITULO
	{ 'Fator Conv.P'														, .T. }, ; //X3_TITSPA
	{ 'Fator Conv.P'														, .T. }, ; //X3_TITENG
	{ 'Fator Conversao Pallets'												, .T. }, ; //X3_DESCRIC
	{ 'Fator Conversao Pallets'												, .T. }, ; //X3_DESCSPA
	{ 'Fator Conversao Pallets'												, .T. }, ; //X3_DESCENG
...
aAdd( aSX3, { ;
	{ 'ZFR'																	, .T. }, ; //X3_ARQUIVO
	{ '37'																	, .T. }, ; //X3_ORDEM
	{ 'ZFR_CODORI'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 40																	, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Cod.Prd.Orig'														, .T. }, ; //X3_TITULO
	{ 'Cod.Prd.Orig'														, .T. }, ; //X3_TITSPA
	{ 'Cod.Prd.Orig'														, .T. }, ; //X3_TITENG
	{ 'Codigo do Produto Origina'											, .T. }, ; //X3_DESCRIC
	{ 'Codigo do Produto Origina'											, .T. }, ; //X3_DESCSPA
	{ 'Codigo do Produto Origina'											, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	{ Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) + ;
	Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) + ;
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP21436" ) ) ) ;
