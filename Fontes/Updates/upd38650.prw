...
/*/
//--------------------------------------------------------------------
User Function UPD38650( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UPD38650" )
				Else
					MsgStop( "Atualização não Realizada.", "UPD38650" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'SC7'																	, .T. }, ; //X3_ARQUIVO
	{ 'ZZ'																	, .T. }, ; //X3_ORDEM
	{ 'C7_I_MEAGE'															, .T. }, ; //X3_CAMPO
	{ 'M'																	, .T. }, ; //X3_TIPO
	{ 10																	, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Agen. Entreg'														, .T. }, ; //X3_TITULO
	{ 'Agen. Entreg'														, .T. }, ; //X3_TITSPA
	{ 'Agen. Entreg'														, .T. }, ; //X3_TITENG
	{ 'Agendamento de entrega do'											, .T. }, ; //X3_DESCRIC
	{ 'Agendamento de entrega do'											, .T. }, ; //X3_DESCSPA
	{ 'Agendamento de entrega do'											, .T. }, ; //X3_DESCENG
	{ ''																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	{ 'x       x       x       x       x       x       x       x       x       x       x       x       x       x       x x', .T. }, ; //X3_USADO
	{ ''																	, .T. }, ; //X3_RELACAO
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPD38650" ) ) ) ;
