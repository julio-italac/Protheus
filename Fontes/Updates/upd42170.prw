...
/*/
//--------------------------------------------------------------------
User Function UPD42170( cEmpAmb, cFilAmb )
Local   aSay      := {}
Local   aButton   := {}
Local   aMarcadas := {}
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização realizada.", "UPD42170" )
				Else
					MsgStop( "Atualização não realizada.", "UPD42170" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'SC1'																	, .T. }, ; //X3_ARQUIVO
	{ '9R'																	, .T. }, ; //X3_ORDEM
	{ 'C1_I_JUERE'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 200																	, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Jus.Eli.Res.'														, .T. }, ; //X3_TITULO
	{ 'Jus.Eli.Res.'														, .T. }, ; //X3_TITSPA
	{ 'Jus.Eli.Res.'														, .T. }, ; //X3_TITENG
	{ 'Jus.Eli.Res.'														, .T. }, ; //X3_DESCRIC
	{ 'Jus.Eli.Res.'														, .T. }, ; //X3_DESCSPA
	{ 'Jus.Eli.Res.'														, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPD42170" ) ) ) ;
