...
/*/
//--------------------------------------------------------------------
User Function UP50805( cEmpAmb, cFilAmb )
Local   aSay      := {}
Local   aButton   := {}
Local   aMarcadas := {}
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgInfo( "Atualização realizada.", "UP50805" )
				Else
					MsgStop( "Atualização não realizada.", "UP50805" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'ZK1'																	, .T. }, ; //X3_ARQUIVO
	{ '37'																	, .T. }, ; //X3_ORDEM
	{ 'ZK1_SUBITE'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 2																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Sub.Tipo'															, .T. }, ; //X3_TITULO
	{ 'Sub.Tipo'															, .T. }, ; //X3_TITSPA
	{ 'Sub.Tipo'															, .T. }, ; //X3_TITENG
	{ 'Sub.Tipo'															, .T. }, ; //X3_DESCRIC
	{ 'Subtipo de Acordo Comerci'											, .T. }, ; //X3_DESCSPA
	{ 'Subtipo de Acordo Comerci'											, .T. }, ; //X3_DESCENG
	{ ''																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	...
	{ 'R'																	, .T. }, ; //X3_CONTEXT
	{ ''																	, .T. }, ; //X3_OBRIGAT
	{ '!Empty(Tabela("ZL", M->ZK1_SUBITE, .F.))'							, .T. }, ; //X3_VLDUSER
	{ ''																	, .T. }, ; //X3_CBOX
	{ ''																	, .T. }, ; //X3_CBOXSPA
	{ ''																	, .T. }, ; //X3_CBOXENG
	{ ''																	, .T. }, ; //X3_PICTVAR
...
aAdd( aSX3, { ;
	{ 'ZK1'																	, .T. }, ; //X3_ARQUIVO
	{ '38'																	, .T. }, ; //X3_ORDEM
	{ 'ZK1_NSUBIT'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 50																	, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Desc. SubTip'														, .T. }, ; //X3_TITULO
	{ 'Desc. SubTip'														, .T. }, ; //X3_TITSPA
	{ 'Desc. SubTip'														, .T. }, ; //X3_TITENG
	{ 'Descricao do Subtipo'												, .T. }, ; //X3_DESCRIC
	{ 'Descricao do Subtipo'												, .T. }, ; //X3_DESCSPA
	{ 'Descricao do Subtipo'												, .T. }, ; //X3_DESCENG
	{ ''																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	{ 'x       x       x       x       x       x       x       x       x       x       x       x       x       x       x x', .T. }, ; //X3_USADO
	{ 'IIF(INCLUI,"",TABELA("ZL", M->ZK1_SUBITE,.F.))'						, .T. }, ; //X3_RELACAO
	{ ''																	, .T. }, ; //X3_F3
	{ 0																		, .T. }, ; //X3_NIVEL
	{ 'xxxxxx x'															, .T. }, ; //X3_RESERV
	{ ''																	, .T. }, ; //X3_CHECK
	...
	{ ''																	, .T. }, ; //X3_PICTVAR
	{ ''																	, .T. }, ; //X3_WHEN
	{ 'TABELA("ZL", ZK1->ZK1_SUBITE,.F.)'									, .T. }, ; //X3_INIBRW
	{ ''																	, .T. }, ; //X3_GRPSXG
	{ '1'																	, .T. }, ; //X3_FOLDER
	{ ''																	, .T. }, ; //X3_CONDSQL
	{ ''																	, .T. }, ; //X3_CHKSQL
...
aAdd( aSX7, { ;
	'ZK1_SUBITE'															, ; //X7_CAMPO
	'001'																	, ; //X7_SEQUENC
	'TABELA("ZL", M->ZK1_SUBITE,.F.)'										, ; //X7_REGRA
	'ZK1_NSUBIT'															, ; //X7_CDOMIN
	'P'																		, ; //X7_TIPO
	'N'																		, ; //X7_SEEK
	''																		, ; //X7_ALIAS
	0																		, ; //X7_ORDEM
...
aAdd( aSXB, { ;
	...
	'01'																	, ; //XB_SEQ
	'DB'																	, ; //XB_COLUNA
	'Subtipos de Contrato'													, ; //XB_DESCRI
	'Subtipos de Contrato'													, ; //XB_DESCSPA
	'Subtipos de Contrato'													, ; //XB_DESCENG
	''																		, ; //XB_WCONTEM
	'SX5'																	} ) //XB_CONTEM
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP50805" ) ) ) ;
