...
/*/
//--------------------------------------------------------------------
User Function UP29374( cEmpAmb, cFilAmb )
Local   aSay      := {}
Local   aButton   := {}
Local   aMarcadas := {}
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UP29374" )
				Else
					MsgStop( "Atualização não Realizada.", "UP29374" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'SB1'																	, .T. }, ; //X3_ARQUIVO
	{ 'Y6'																	, .T. }, ; //X3_ORDEM
	{ 'B1_I_PRDSM'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 200																	, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Prod.Similar'														, .T. }, ; //X3_TITULO
	{ 'Prod.Similar'														, .T. }, ; //X3_TITSPA
	{ 'Prod.Similar'														, .T. }, ; //X3_TITENG
	{ 'Lista Produtos Similares'											, .T. }, ; //X3_DESCRIC
	{ 'Lista Produtos Similares'											, .T. }, ; //X3_DESCSPA
	{ 'Lista Produtos Similares'											, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
aAdd( aSXB, { ;
	...
	'01'																	, ; //XB_SEQ
	'RE'																	, ; //XB_COLUNA
	'Produtos Acabados'														, ; //XB_DESCRI
	'Produtos Acabados'														, ; //XB_DESCSPA
	'Produtos Acabados'														, ; //XB_DESCENG
	''																		, ; //XB_WCONTEM
	'SB1'																	} ) //XB_CONTEM
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UP29374" ) ) ) ;
